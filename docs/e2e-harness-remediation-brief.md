...
**Process topology (reuse the PROVEN pattern verbatim from `api/tests/journeys/boot-b.mjs` and
...
transport (or `api/tests/journeys/_lib.mjs` `api()` shape): each row below is a plain HTTP call
...
The Boot-A probes (`api/tests/journeys/*.mjs`, `_lib.mjs` helpers) are standalone Node scripts that
hit `:4111` and emit `PASS|FAIL|INFO <id>` + write evidence to `api/tests/evidence/<journey>/`
...
- **Credential-less lane `npm run journeys:credless` (per-PR capable).** New `scripts/journeys-run.mjs`
  ...
  Evidence regenerates under `api/tests/evidence/`.
- **Credentialed lane `npm run journeys:credentialed` (opt-in; NEVER a per-PR CI gate).** Boots
  `api/tests/journeys/boot-b.mjs` (reads the operator's Claude Code OAuth from the macOS Keychain,
...
4. `npm run journeys:credless` exits 0 on a clean checkout, 6 probes run, zero `FAIL` lines; evidence
   regenerated under `api/tests/evidence/`.
