...
prompt-injected only; no agent-callable memory tool exists (`api/src/agents/tools.ts` lists knowledge tools only).
...
capture_enabled (true), register_codex_gemini (true; false in all five live compositions). Setup installs upstream
...
sk-*/ghp_*/xoxb-*, writes session-*.md with YAML frontmatter into the vault; bare-except + always exit 0.
No LLM call. Consolidation ("dream") lives in the Improver fitting, not here.

### 2.2 Voice fitting

**The plan's premise "partly converted to Ekoa" is FALSE in this repo.** The fitting is
...
server was retired into Muster's policy panel — `garrison:src/lib/orchestrator-policy.ts`). **No bridge code exists
in garrison** (zero hits for delegate_to_local/bridge tokens — rule 9 verified; the bridge lives Cortex-side under
`api/src/bridge/`). → the Garrison view over Cortex automations is a NEW minimal connector fitting (slice G5),
...
Current state: §2.2 (garrison: pure Deepgram, done, provider-agnostic consumer) and ekoa-code: no voice capability
module exists in api/src (the eventual capability would be net-new; nothing half-built to reconcile).
Sketch of a future contract (shape only): ops transcribe (audio → text; batch POST, binary kind) and synthesize
(text → audio stream), plus session-scoped streaming — but the defining feature is REAL-TIME bidirectional
streaming (WS /stream with silence endpointing; WS /tts-stream with barge-in `speak|flush|clear`), which the
current descriptor mechanism models only nominally (kind: 'ws' exists in `shared/src/descriptor.ts`, but zod
request/response schemas do not describe frame protocols) and OpenAPI models poorly (AsyncAPI territory).
Open questions for the operator (recorded, not answered):
1. Does voice belong in the HTTP capability contract at all, or is it a separate streaming contract (AsyncAPI/
   frame-schema doc) beside it?
2. Per-user metering for streaming minutes: the gateway meters tokens today; voice needs a duration/bytes meter —
   new billing lane or out of scope for the capability plane?
3. Latency: garrison's fitting is same-host by design (voice-latency logging); a hosted Cortex hop adds RTT — is
   remote voice acceptable for the daily proving ground, or does voice stay local-only with Cortex as key broker?
4. Provider abstraction: keep Deepgram-shaped ops (nova/aura vocabulary) or neutral ops with provider config?
