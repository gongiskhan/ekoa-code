...
- **W5 §12.6 privacy surfaces DONE** (commit e9a6804): all 13 FC-400..412 built - composer attach (Upload/Reference, 3 bridge states), trust chip + custody panel, "Privacidade e ponte local" settings surface (absorbed /settings/bridge; bridge status/pairing, grants+revoke, ledger viewer, masking summary, unified approved-commands, grounded sections), first-grant dialog, onboarding card. CLAIMS SHIP-GATED (lib/privacy-claims.ts CLAIMS_SHIP_GATED=true -> <GatedClaim> "verificação em curso"). VERIFIED forbidden-phrase scan 0 (every claim ceiling-bound + qualified), no emoji, PT-PT. typecheck 0, build 0, unit 103/103.
- **W5 REMAINING (the G9 gate):**
  1. e2e (FC-312/306): run `npm run e2e:server` to see which platform specs break on the migrated UI; rewrite the protocol-coupled specs vs the typed UI + the no-mode-picker guard. The 37 band3 served-app specs should stay green (unaffected by the platform migration). Deflake as needed.
  2. Diagrams (FIXED-12, §12.8 crit + ch14 gate item 5): update spec/diagrams/02-module-map (web now has lib/api/ - the typed client + stream + canvas modules, no lib/cortex/connection), 03-request-crud (the request core path), 04-agent-job (the SSE stream consumer + the canvas WS) to match the as-built transport. A structural change without its diagram update is incomplete.
  3. THE GATE (ch14 template): single `npm run ci:lane` exit 0 (shared+api+web typecheck/lint/build, greps incl. the 17 §12.8 criteria grep-gates: legacy-transport census 0, one token accessor, one base-URL, EventSource->stream.ts, WebSocket->canvas.ts, teams removed, ekoa_locale gone, forbidden-claims 0), e2e:server green, the FULL ledger (Playwright + unit + drivers + protocol-parity + shared allowlist EMPTY), dual review (Claude fresh-context + the deferred Codex cross-model once credits return), checkpoint commit + tag gate-9.
