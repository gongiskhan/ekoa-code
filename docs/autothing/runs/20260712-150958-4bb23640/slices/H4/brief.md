...
## FINALIZED 2026-07-13 (post-H3)
- **Convert = dashboard starts the patch run directly.** H3's edit entry is a PANEL flow, but the
  org-admin queue lives in the web DASHBOARD (platform, same origin, has the admin JWT). So
  "converter" on a queued request POSTs /api/v1/jobs { artifactId, description: request.text }
  directly (H1-gated: the org-admin has canEditApps + loadWritable on an org app) and marks the
  ChangeRequest converted (status:'converted', + the jobId). NO deep-link into the served app
  needed. The dashboard can show the resulting job/preview like any build. (Edit mode in the panel
  stays the in-app path; the queue convert is the dashboard path - both drive the same H1-gated
  follow-up build.)
- **Filing requires a logged-in platform user.** The queue is org-internal; an anonymous served-app
  visitor has no platform identity to route/authorize. The panel "Pedir alteracao" button reads the
  platform token (same defensive read as H2/H3); if absent (not logged in / cross-origin), the
  button either hides or explains "inicie sessao no Ekoa para pedir alteracoes" - decide + document
  (recommend: show but on submit require the token; a 401 -> that calm message). The filed request
  carries requesterUserId from the verified token, appId + owner org from X-Ekoa-App-Id, and the
  captured route/screen context.
- **Cross-org isolation is the security crux (H5 will assert it):** a ChangeRequest is scoped to the
  app OWNER's org; GET /api/v1/change-requests returns ONLY the caller-org's requests for org-admin
  (super-admin across orgs), mirroring registo.ts exactly. An org-admin MUST NEVER see another org's
  requests. Contract + integration test this explicitly.
- Refused-build feed (H1 details.capability): when a user's chat build is refused (canBuildApps),
  offer to file a ChangeRequest pre-drafted with the original description to the org-admin queue -
  the machine hook (details.capability + the description) already exists; wire a thin path (client
  or a small server helper) so a refusal is never a dead end. Keep it minimal; do NOT add capability
  logic.
