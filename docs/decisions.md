...
## 2026-08-21 - D-S4-5/D-S5-8: the document an agent reads, the page a person reads, and the record of what a model chose

**MAJOR 1: THE SHIPPED SKILL DOC STILL DESCRIBED THE PRE-COMPOSE CONTRACT.**

`clients/cortex-cli/SKILL.md` said, verbatim:

    `achieve` has three outcomes and only one of them is exit 0:

with a three-row table holding `executed`, `authored` and `refused`. `composed` had already landed
in `shared/src/integrations.ts`, in `docs/openapi/cortex.v1.json`, in the generated client, in the
route and - last round - in this command's own exit-code branch. The document did not move.

That is not cosmetic and it is not a typo. SKILL.md is the file an AGENT reads to decide how to use
the command, so a stale contract there is a wrong INSTRUCTION shipped to every consumer: an agent
believing it treats a successful narrowed read as a failed goal, which is precisely the defect the
client itself had last round. It is the same class as the route docblock and the descriptor on the
sibling branch - the code moved and the thing people read did not.

The doc is now TESTED rather than proof-read, against the two authorities it describes
(`clients/cortex-cli/tests/integrations.test.ts`, "the shipped skill doc describes the contract this
command implements"):

  1. WHAT OUTCOMES EXIST is read from `docs/openapi/cortex.v1.json` - the published contract this
     package generates its client from. Every outcome the spec declares must have a table row.
  2. WHAT EACH ONE DOES is taken from RUNNING THE COMMAND. For every outcome the spec declares, the
     binary is driven over a stubbed response and its real exit code is compared against the exit
     column of that outcome's row. The doc cannot drift from the behaviour in either direction: a
     client change without the doc, and a doc change without the client, both red.

The doc also gained TRAP 3 (`composed` is an answer and `items` is not the whole of it), the
`ladder` vocabulary, and where a model's chosen VALUES are and are not readable.

**MAJOR 2: HUMAN-MODE `composed` DROPPED THE ACTION'S ENVELOPE, SO A PAGINATED 200 READ AS THE
WHOLE ANSWER.**

The third variant of this rung's sharpest defect, and the most dangerous of the three, because the
caller cannot see it. D-S5-7 put the arm's whole answer on `result` at the module, projected it at
the route and printed it under `--json`. `printComposedResult` - the HUMAN path - printed `items`
alone:

    ok  crm/processos
    composed  2 of 4 rows kept, joined against "clients"
    [ ...2 rows... ]

Every field standing BESIDE the list inside `data` - a `nextPage` cursor above all, a `total` -
never reached the ONE reader who cannot go and look it up. The composed line tells the reader the rows were NARROWED; nothing told them what
they were narrowed FROM was itself one page, so a join over the first page of a paginated read
printed identically to the same join over the whole of it. For "todos os processos
de clientes com menos de 40 anos" that is a lawyer reading a partial case list as the full one.

`execute` has always printed the action's `data` whole, so the fix is that rendering plus the
narrowing rather than a new dialect: the envelope is printed, labelled as the action's own answer,
ABOVE the rows - `items` is capped at 200, and a partiality signal underneath 200 rows of JSON is a
signal nobody reads. Nothing is summarised or filtered on the way: deciding WHICH siblings of the
list matter would mean this client guessing which key a given third party paginates with, and the
one it guessed wrong about is the one that would go missing.

THE FIXTURE FOR IT ANSWERS HTTP 200, deliberately. A 206 announces itself in the status line the
client already printed; a paginated 200 - which is what most real list endpoints answer - announces
itself NOWHERE except in the envelope.

**MINOR 1: `composition.scanned` WAS UNPINNED ACROSS THE WHOLE ESTATE.** Every fixture held four
action rows and four collection rows, so `scanned` (4) was indistinguishable from `collectionScanned`
(4), and `matched` from `matchedCollectionRows` (2 and 2). No assertion anywhere could say which
SIDE of the join a count was counting, and a summary reporting the caller's collection size as the
action's page size would have been green everywhere. A fifth client (`c5`, 25, keying no process)
is added to all three fixtures - the module suite, the contract suite and the cortex-cli e2e - so
the canonical answer now reads scanned 4, collectionScanned 5, matchedCollectionRows 3, matched 2:
four counts, four different numbers, asserted at the pure stage, on the wire and through the client.

**MINOR 2: THE HUMAN'S ONLY TRUNCATION WARNING HAD NO TEST.** `- PART of the answer, not all of it`
is the one thing on screen that says a narrowed list is not the whole of somebody's answer, and it
could be deleted, inverted, or reduced to one of its two flags with the estate green. It is now a
four-row table of the rendering (both flags, each alone, both, neither) plus an end-to-end case
driving 201 real rows through the real join stage so `truncated` is set by `composeRows` counting
past its own emit cap.

**MINOR 3: `mint` AND `ladder`-ON-A-REFUSAL WERE PUBLISHED AND POPULATED BY NOTHING.** `AchieveRung`
has carried the word `mint` since the ladder shipped and no code pushed it; `AchieveResult` declared
`ladder` on `authored` and on `refused` and no call site passed it. A vocabulary a server publishes
and never emits is a contract the code does not have. Answered per field, which is what
"populate it or stop publishing it" means here:

  - `mint` IS PRODUCED. The author arm - the only code in the platform that writes a new action - is
    the mint rung, and now says so. An `authored` answer carries exactly two steps: `reuse`
    `skipped` (nothing fitted, which is WHY anything was minted) and `mint` `taken`. `parametrize`
    and `compose` are deliberately absent: both act on a MATCHED action's arguments and rows, and
    there is no matched action on that arm, so a step for either would report a decision nothing
    took. `ladder` is REQUIRED on `authored` rather than optional.
  - `ladder` ON A REFUSAL IS GONE FROM THE TYPE. Not left declared-and-empty: the refused variant
    has no field to put one in, so "present on exactly the outcomes that carry an answer" is a
    property of the shape instead of a sentence somebody has to keep true. A refusal is the answer
    that NO rung produced anything; what went wrong is `code`/`message`/`violations`, which every
    client already reads.

RULE 7: `ladder` is unchanged on the wire schema (optional on one flat object, as it has always
been) and no enum member moved. `mint` is a value a consumer could already receive under the
published schema and never did; `authored` gains a field it never carried. `gate:openapi` and
`gate:client-drift` both run and produce no diff.

**MINOR 4 - THE ONE THAT MATTERS MOST: THE RECORD OF A MODEL'S CHOICE WAS WRITTEN AFTER THE
IRREVERSIBLE WRITE AND SWALLOWED ON FAILURE.**

`capability_achieve_parametrize` is the only durable record that a MODEL, rather than the person or
script holding the key, decided what a third-party call would act on. It was awaited in
`runMatchedAction` AFTER the one gated execute, inside its own `catch`, which logged and moved on.
So the audit trail for "what did the model pick for this write" could be silently absent EXACTLY
when the write had succeeded - missing precisely on the runs that mattered, which is not a trail.

Both halves are fixed by one move: the write goes BEFORE the execute (`recordParametrization`,
awaited by `parametrizeArgs`) and no longer catches. Nothing has been spent at that point, so the
failure has somewhere to go that is neither silence nor a destroyed answer.

REFUSING THE CALL WOULD HAVE BEEN THE OTHER DEFECT - the parametrize rung taking away a call that a
trusted action, the caller's own arguments and a human's standing approval had already earned,
because an activity insert blipped (D-S4-3/D-S5-6). So the rung STANDS DOWN, on its own existing
degradation path: the model's arguments are dropped, the request goes out carrying exactly what the
caller sent, one ladder step says `unavailable` ("we could not right now" - nothing about the goal
was judged), and the answer reaches the caller whole. THE RECORD OF THE CHOICE IS A PRECONDITION OF
THE CHOICE: if it cannot be recorded, the choice is not made, and a model-chosen value can no longer
reach a third party unrecorded.

THE OUTCOME LEAVES THE ROW, and that is what made the move affordable. `verdict`/`code` were copied
off the executor's result onto this row, which is the same fact `capability_execute` already records
for the same call one insert later - two rows carrying one fact is two rows that can disagree, and
it was the ONLY reason this row had to wait for a write to finish. An auditor reads the pair: this
row says what a model chose, that row says what happened when it was spent. Both are still written
when the write gate refuses. The suites assert the PAIR rather than the deletion, because "we
dropped a field" and "we lost a fact" are different claims and only the first one is true.

**MINOR 5: `promptSafeFields`'s FILTER-BEFORE-SLICE ORDERING WAS UNPINNED - AND THE OBVIOUS REASON
FOR IT IS FALSE.** The review put it as "the ordering is what stops an unsafe field name reaching
the prompt". THE CODE DISAGREES AND THE CODE WINS: `.slice().filter()` sanitises everything it emits
exactly as `.filter().slice()` does, and no name the predicate rejects reaches a system prompt under
either order. What reversing them changes is WHOSE names the cap is spent on. Capping first spends
the 100 slots on the RAW list, refused names included, and then discards some of them - so the
caller is offered FEWER than 100 usable names whenever a rejected one sorts early. Both sets arrive
sorted and a C0 character sorts below every letter, so a remote emitting a hundred control-charactered
keys leaves the caller an EMPTY offered set and a rung that can narrow nothing at all: a third party
deciding what this tenant may narrow their own answer by. The docblock now states that consequence
and explicitly disclaims the injection one; the test is a 100-refused + 100-good fixture that reads
`[]` under the reversed order.

## 2026-08-20 - S2: the integration detail page, and evidence stays off the key surface

**THE SURFACE.** `/integrations/[key]` is the new home for what `/automations/[id]` provides today,
scoped to what an integration DOES: the actions list with its backing chip and the write gate's own
consent state; the read-only steps per backing (an api-call action's method + URL template off the
definition, a browser-steps / bash-cli action's bound automation plan with the S1 evidence run's
screenshots and output resolved onto each step); the run history; the schedules aimed at that action;
and run-now. The LIST page keeps connection and consent management, and its rows link into the detail.
Nothing here is a second authoring path: an action's steps live on a definition document or on an
automation, each with its own save gate, so this view shows and links and never writes a step.

**ONE NEW ENDPOINT, AND IT IS `user`.** `GET /api/v1/integrations/:key/evidence` ->
`{ items: IntegrationActionEvidence[] }`. Emphatically NOT `user-or-key`, on the `listRecipes`
reasoning applied to a strictly more sensitive artefact. A recipe is a tenant's learned MAP of a
portal's private API and that alone kept it off the key surface; an evidence row is that tenant's real
REQUEST and real RESPONSE BODY - client names, processo numbers, invoice totals. A key-bearing agent
that could read this could walk every action of every integration its user holds and pull the tenant's
actual portal data out over an API, while gaining no capability it lacks: `executeAction` runs the
action either way. Narrow is the reversible direction - widening an auth class later is additive under
Rule 7, narrowing one is not. The consequence is visible in the gates: `npm run gate:openapi` and
`npm run gate:client-drift` are both clean with no regenerated bytes, because the spec is
DEFINITIONALLY the `user-or-key` surface and this endpoint is absent from it by construction.

**THE READ IS OWNER-SCOPED, AND THAT IS THE FIRST SCOPE.** S1's collection is keyed by
`(orgId, ownerUserId, integrationKey, actionName)` - the re-key S1's own round-eight entry above
records as "the sample belongs to the owner" - so a row is ONE person's record of ONE third-party
session. `listForIntegration(orgId, ownerUserId, integrationKey)` takes both identity terms, and this
read passes the caller's own `actor.orgId` and `actor.userId`, never a user id read off a request.
The consequence is deliberate and is written down rather than left to be discovered: two members of
one org open the same detail page and see DIFFERENT evidence, each the record of the account THEY ran
the action with. The alternative - an org-wide read narrowed by what the reader can see - was
considered and rejected here: it would hand one member a colleague's real request and real response
body, which is the tenant's actual client data, and no filter over an answer can un-read that.
Narrow is the direction that stays reversible.

**AND THE DEFINITION READ PREDICATE IS THE SECOND, which is why the read is a module rather than a
route body.** The two keys move INDEPENDENTLY: a row is addressed by an action NAME, and the package
naming that action is a separate document with its own lifecycle, so a caller's OWN rows outlive the
definition that produced them - in two ordinary ways. An action is RE-AUTHORED OUT of the package,
and its row stays until it is superseded, discarded or aged out at 90 days. Or the caller's
RESOLUTION changes under them: `getForActor` answers a reader their OWN `private` row before any
`org`/`global`/baseline one, so gaining a private package - or having an org one replaced by a
narrower revision - silently narrows what they resolve while their older rows stay exactly where they
were. In both cases a straight collection read renders one real request and one real response body
beside an action the caller can no longer see, cannot run and cannot name.
`api/src/integrations/action-evidence-view.ts` therefore resolves the definition FIRST, under the
caller's own actor, through the same `resolveCapabilityDefinition` the capability read and `achieve`
use (imported, never re-derived), and keeps only the rows whose action is on THAT definition. That is
`recipe-store.listRecipesForActor`'s own rule applied to the sibling collection. It is a SEPARATE
FILE from `integration-capability.ts` on purpose: that module is the `user-or-key` capability core,
and keeping the two apart means nobody can make this collection key-reachable by adding a field to a
view the core already returns.

**THE PROJECTION DROPS THE STORAGE ENVELOPE** (`_id`, `orgId`, `ownerUserId`, `integrationKey`) and
ADDS nothing:
every field on the wire is a field S1 wrote through the executor's own redaction and the store's
whole-document last gate. `shape` IS on the wire, and earns its place - it is the action fingerprint
the run exercised, so a client can say "this sample was recorded before the action was last changed"
instead of presenting last month's request as evidence for today's action. It is a hash, not a body,
and it is the same token `approveAction` keys on.

**THE WEB QUALITY BAR, TAKEN FROM THE SCHEDULES DEFECTS RATHER THAN FROM PRINCIPLE.** Every rule
below was a real defect on the schedules surface in this repo, and each is pinned by a case that was
verified to go red against a deliberate mutation of the source:
- a failed READ never renders as an empty state. The samples, the steps, the history and the schedules
  each render their own failure where the data would have gone;
- the page's read keeps the `ApiError`, not a string, so a 404 (absent - and refused, and cross-tenant:
  one uniform envelope) is told apart from a 500/403/network (failed to load something still there);
- there is no infinite spinner: "not back yet" is structurally distinct from "came back broken" and
  from "came back empty" in every one of those four sections;
- a MUTATION failure is visible at the point of the click. `runNow` has three failure shapes and the
  middle one is the one a naive `if (res.ok)` loses whole - the execute endpoint answers 200 with
  `success: false` for a remote 500, a locked credential, a disabled integration and a timeout;
- run-now is rendered ONLY where the server would admit it, mirroring the capability row's own
  `connected` / `requiresApproval` / `approved` (never a second reading of `mutates`, which lives in
  `action-consent.ts`); where it is withheld the row still reads and states the reason, with a real
  link to the page that can fix it;
- every failed section offers a RETRY where it failed, so recovery is never "reload the page";
- NO COPY IS WRITTEN IN THE STORE. Each failure channel carries the fact plus, at most, the server's
  own prose; the words come from `locales/` in the reader's language. `runNow` carries the
  executor's `code` as a token to be TRANSLATED, never as text to be printed - the posture
  `shared/src/run-errors.ts` established, applied to the integration executor's vocabulary;
- the stale-key guard is on EVERY read, not only the page's own: `steps` and `runs` are keyed by
  action name while the unit of the decision is `(integrationKey, actionName)`, and two integrations
  routinely declare the same action name;
- the evidence step samples are joined onto the plan ONLY where something identifies the run that
  produced them (its `runId`, found in this automation's own history) or, failing that, where they
  at least ADDRESS the steps on screen - and in that second case the page says the samples may be
  from an earlier version rather than presenting them as evidence for today's steps. Neither
  `RunRecord` nor the row carries a plan hash and `shape` fingerprints the BINDING, so run identity
  is the ceiling and is documented as such;
- an action's schedules count BOTH target kinds: a `kind: 'automation'` schedule aimed at the bound
  automation fires the very runs this page's history attributes to the action, so counting only
  `integration_action` told a person "this action is not scheduled" about an action that runs every
  morning;
- the list card's navigation into the detail is a real `<Link>`, not a handler on the card div.

**A LATENCY CHOICE MADE EXPLICIT.** `load()` fires the samples read but does NOT await it before
committing the capability: a slow section must not hold the whole page on a spinner. The promise is
still awaited at the end so `load()` resolves on the full settle. Pinned by a case that renders with
the samples read permanently outstanding.

**REVIEW ROUND (2026-08-22), AND WHAT IT CORRECTED IN THIS ENTRY.** The first draft of this slice
was written against a SUPERSEDED revision of S1's collection - the org-only key of an earlier round
of the same unmerged branch - and rebased forward without being reconciled. Everything above about
scope said "keyed by the org and NOT by the user, so two members of one org share a row", the view
called `listForIntegration` with two arguments against a three-argument signature (so the branch did
not typecheck and the endpoint would have answered every caller an empty list), and both new suites
seeded rows with no `ownerUserId` - a shape `assertKey` only accepts because its guard compares
against `''` and `undefined` slips through. This entry is corrected IN PLACE rather than appended to,
because it is part of the same unmerged commit: what is above is now what the code does. The
correction is deliberately the NARROW one - the read passes `actor.userId` and stays owner-scoped -
and not the org-wide-read-filtered-by-visible-actions design the first draft's own module header
described, which would have handed one org member a colleague's real request and response bodies.
The definition filter is kept and re-justified against the owner-keyed store (a row outlives the
package naming its action), and the Rule 5 suite is rebuilt so its central case is same-owner: an
evidence row for an action the caller's own resolution no longer carries. Recorded here rather than
in the commit message alone because the wrong version of this paragraph is what a future editor would
have followed.

DIAGRAM CHECK (FIXED-12): DONE, append-only. `docs/diagrams/02-module-map.excalidraw` gains the S2
read module, the two scopes it enforces and the detail page's client modules (route, store, view
...
## 2026-08-23 - S8 live pass: the capability tells a client which automation is THEIRS

**WHAT THE LIVE PASS CAUGHT, and it was a product defect rather than a spec one.** The rewritten
`integration-session-automations` case drove the citius detail page for the first time on a real
boot, and the steps section read "Automação não encontrada" beside four automations that plainly
existed. The cause is one identifier meaning two different things.

A package declares its binding as `automationBinding: { automationId: 'citius-notificacoes-template',
automationTemplate: 'notificacoes' }`. That `automationId` is a PLACEHOLDER its author wrote: it
names nothing, in any org, ever. The row a tenant actually runs is minted by provisioning under
`managedAutomationId(orgId, integrationKey, templateKey)` - a per-org sha256 - and the only join back
is PROVENANCE, `source.{integrationKey, templateKey}`. `web/lib/integrations/action-view.ts` read the
declared id straight off the definition and fetched it, so every automation-backed action on every
shipped package resolved to a 404.

**A CORRECTION TO THE REPORTED DIAGNOSIS, because it changes where the fix belongs.** The review
brief said the capability wire "NEVER carries automationBinding" and that the field does not exist in
`integration-capability.ts` or `shared/`. The first half is not so: the capability puts the whole
resolved definition on the wire as `integration`, and its raw action records DO carry
`automationBinding` - verified against the running stack, which returns it for all four bound citius
actions. What is true is the second half: nothing on the TYPED capability action carried an id, and
the id that was on the wire was the useless one. So this is not a missing field being added for the
first time; it is a WRONG VALUE being replaced by a right one, and the fix has to compute something
the definition cannot know rather than merely forward something it already has.

**THE SHAPE, minimal and honest.** `IntegrationCapabilityAction` gains an OPTIONAL `automationId`
(Rule 7 additive; the OpenAPI delta is three lines and the generated client regenerated with it).
Not an `automationBinding` object: the client needs exactly one thing, and echoing the binding would
re-expose the placeholder beside the real id and invite the same mistake again.

ABSENT MEANS TWO THINGS AND BOTH ARE HONEST: an api-call action has no automation, and a bound action
this org has not provisioned has no row yet. The id is deterministic, so it COULD be computed for a
row that does not exist - and emitting it would turn "nobody has prepared these steps" into "here is
an automation" and a 404 one fetch later. Only materialised rows are reported.

**IT IS A SEAM, NOT AN IMPORT.** `integrations/` may not reach into `automation/` - the S1 evidence
collector is the standing precedent - so `CapabilityContext` gains `resolveManagedAutomationIds`,
bound at the composition root to `managedAutomationIdsFor`, exactly as `platformConnected` is. One
query per capability read, not one per action. Unbound or failing, every action reports no automation
id, which reads as "not provisioned yet": the degradation is the same state the product already has a
rendering for, never a wrong id.

**THE FIXTURE-HONESTY CLASS, AGAIN, AND THIS IS THE PART WORTH REMEMBERING.** Every S2 hermetic test
passed throughout, because both mocked a capability whose `automationBinding.automationId` WAS the
org's real id - a shape no production writer can emit. The mock did not merely fail to catch the bug;
it encoded the bug's inverse as the expected world. `slack` masked it in the S2 live pass because an
api-call package has no binding at all. Both fixtures now carry the placeholder on the definition and
the real id on the typed action, with the two values DELIBERATELY DIFFERENT, so a reader that goes
back to the binding fails five tests instead of passing them.

**GATES.** `api/tests/contract/integrations-capability.test.ts` gains three cases against the real
assembly - a bound action reports the org's id and not the declared one, an api-call action reports
no id at all (asserted as key-absence, not empty string), and an unprovisioned bound action reports
none rather than one that 404s. Mutation-proven three ways: dropping the view's projection reddens
the contract suite, keying the lookup on the declared id instead of the template reddens it, and
restoring the client's old binding read reddens five S2 cases. `gate:openapi` and `gate:client-drift`
are clean after committing the regenerated bytes.

**TWO SPEC DEFECTS FROM THE SAME PASS, fixed without touching the product.** `coherence-locale` and
`pages-core` moved onto `/integrations` in S8 and inherited a console-error assertion that counts raw
`Failed to load resource` lines - which carry no URL. `/integrations` legitimately makes one 404: the
CS6 sync-state endpoint with its flag off, which `citius-sync-outcome.spec.ts` already documents as
the behaviour under test. Both now use that sibling's pattern: ignore the raw line, pin non-2xx by
RESPONSE URL, allow exactly that one address. Every other 404 still fails, which the blanket
`favicon` filter they had could not say.

**AND ONE DISPOSITION, TAKEN THE HONEST WAY.** `automation-deterministic` fails on a daemonless boot
because the engine resolves every step's locality through `egressRequirementFor` and an UNDECLARED
origin is bridge-only (P4) - so a `wait` step that touches no network halts with "no machine is
paired to your account". THE OBVIOUS FIX IS NOT AVAILABLE: a per-step `declaration` would make it
cloud-safe, but `POST /api/v1/automations` builds a step from `{stepId, description, tool}` alone and
`automation/service.ts` states why - a Cofre-referencing declaration is a NEW POWER on a
`user-or-key` surface, not a new spelling of an existing one. Declaring it in a spec payload would
need an auth-class change, which a spec fix must not make. So the spec is SPLIT: one case asserts
what holds on any boot (the plan is accepted, a run is created, the engine drives it off `running` to
a terminal status, and step 0 carries a record that is not pending) and deliberately does not claim
the steps succeeded; the second keeps the original claim whole - BOTH steps completed - as a
`test.skip` whose reason is in the file and in the ledger. The assertion was NOT weakened to tolerate
a failed step. Unblocking it means a paired machine on the runner or a posture decision about
undeclared origins.

DIAGRAM CHECK (FIXED-12): DONE, append-only. `docs/diagrams/02-module-map.excalidraw` gains the
...
- 2026-08-26 - D-BIND-FAMILY: a captured session binds to the login's OWN auth-domain family, not a single host. `boundOriginsForEstablishedHost` (`api/src/cofre/sessions.ts`) previously returned `[host]` - the single ceremony/typist host. That broke every real multi-domain login: Uber Eats authenticates across `ubereats.com` (app session) AND `uber.com`/`auth.uber.com` (SSO session), and a session bound only to `www.ubereats.com` was UN-DISCOVERABLE by `findSessionItemsForOrigin` the moment a step's origin resolved to `auth.uber.com`, so the run re-halted `needs_credentials` on a session that existed but could not be found (findings.md, `ad-hoc-captured-session-does-not-authenticate-a-multi-domain-login`). CORRECTION of the original diagnosis: the narrowing never dropped cookies - the whole jar is stored and injected; `boundOrigins` gates item DISCOVERY, not cookie injection. THE RULE: bind to the domains the server itself scoped a session cookie to - every `httpOnly` cookie in the jar, at its server-declared domain, plus the established host, collapsed to the broadest binding per family. `httpOnly` is the discriminator because it is the signature of a server-managed session/auth cookie; analytics and marketing cookies, being JS-set, are never httpOnly, so the item stays un-discoverable under `google-analytics.com` - the exact property the earlier narrow fix (against the confused/compromised-daemon over-binding threat) was defending, now extended to include the login's real cross-domain auth family. ACCEPTED TRADEOFF: a login federated through a shared third-party IdP binds the item to that IdP's domain too (the server set an httpOnly cookie there). Bounded - items are owner-scoped, grant-gated, and only discovered for an origin a run actually navigates to; an IdP denylist is a later refinement, not a correctness gap. Both entry paths (typist capture, ceremony `session.push`) use the one function, so they bind identically.

- 2026-08-26 - D-CEREMONY-REALCHROME: the attended ceremony opens a NORMAL persistent real-Chrome window, not throwaway bundled Chromium. The operator, after completing a live ceremony, called the experience "very bad ... a normal user would not be able to cope with this": the window flapped tabs, stole focus on every navigation, and showed the "controlled by automated test software" banner while they typed a real password, and they could not reach another app for an OTP. Root cause: `clients/bridge/src/attended/ceremony.ts` launched Playwright's BUNDLED chromium via `chromium.launch()` into a THROWAWAY context, a different and worse path than the run executor's, which already launches the user's real installed Chrome on a dedicated persistent profile (`browser/profile.ts`). THE CHANGE: the ceremony now opens through a shared `browser/chrome-launch.ts` (`launchHeadedRealChrome`) - real Chrome first (`channel:'chrome'`, bundled fallback), a dedicated per-origin `--user-data-dir` under `<EKOA_BRIDGE_HOME>/ceremony-profiles`, the "controlled by automated software" infobar suppressed (`ignoreDefaultArgs:['--enable-automation']`), the webdriver tell removed, navigated ONCE and then left alone (no re-goto, no second tab). Wins: no automation banner, a real Chrome window, the login persists (login-once-per-site), and capture now happens in the SAME real-Chrome environment replay runs in - which is the other half of why a hard adversarial SSO (Uber) failed to authenticate on replay (see D-BIND-FAMILY). WHY NOT AN EXTENSION (the operator asked explicitly for a non-extension path): every other agent reached for a browser extension because the frictionless "attach to the user's EVERYDAY browser" path is a security boundary Chrome closed in Chrome 136 (May 2025) - `--remote-debugging-port`/`--remote-debugging-pipe` are ignored on the DEFAULT `--user-data-dir`, precisely to stop the cookie-theft/infostealer pattern. A dedicated profile (which is what both this and the run executor use) is the only supported way to drive real Chrome; the cost is one login per site. WHY NOT force ceremony and replay to share ONE on-disk profile: the run executor's lease wipes the jar on release and has an idle reaper and a per-profile SingletonLock mutex; a human-paced ceremony sharing that dir would be wiped mid-login or have its window reaped while they fetched an OTP. So the launch PRIMITIVE is shared, the LIFECYCLES stay separate, and continuity travels through the captured storageState (now bound to the full auth-domain family). RESIDUAL, still open: the window opens on the BRIDGE machine, which may not be where the human sits - a fully frictionless flow wants the window on the human's own machine (own design pass, findings.md).

- 2026-08-26 - D-BIND-NARROW-2026-08-26: CORRECTS D-BIND-FAMILY (same day). The family-widening in D-BIND-FAMILY - bind a captured session to every domain carrying an httpOnly cookie in the jar - was caught by adversarial cross-model review as a SECURITY REGRESSION and is reverted. THE FLAW: on the ceremony `session.push` path (`api/src/bridge/attended.ts`) the DAEMON declares the whole `storageState` (`z.unknown()`), so the jar's `httpOnly` flags are attacker-controlled. A compromised daemon could push `{domain:'bank.example', value:<attacker session>, httpOnly:true}` alongside the legit ceremony cookie; the item would bind to `bank.example`, and the owner's own later run targeting `bank.example` would discover it and inject the attacker's jar - session fixation into the owner's automations. A `{domain:'.com', httpOnly:true}` cookie would bind to every `.com` (no public-suffix guard). This re-opened the exact confused-deputy hole the prior narrow fix (`attended.ts` docblock, review round F1) had closed. THE CORRECTION: `boundOriginsForEstablishedHost` binds to `[host]` - the single origin CORTEX asserted (the ceremony's origin, or the typist's integration host) - and never to any domain derived from the daemon-declared jar; the jar remains EVIDENCE only (a cookie must cover `host`). WHY THE WIDENING WAS NEVER NEEDED: `boundOrigins` gates item DISCOVERY, not cookie injection - the whole jar is injected at reuse regardless (`automation/local-browser-session.ts` `addCookies` takes every cookie), so a multi-domain login's cross-domain auth cookies ride along and a redirect to `auth.uber.com` is already authenticated; and a run halts at the SPECIFIC origin it needs, so Cortex opens the ceremony for exactly that origin and the narrow binding matches. A later step that targets a second registrable domain DIRECTLY re-ceremonies, which is instant on the persistent real-Chrome profile (D-CEREMONY-REALCHROME). THE REAL FIX for the live Uber Eats failure was therefore the real-Chrome capture/replay environment continuity, not a wider binding - my original diagnosis (D-BIND-FAMILY) was wrong on both the cause and the remedy. Adversarial tests added: `cofre-sessions.test.ts` (binds to exactly the asserted host; ignores a crafted httpOnly rider for an unrelated domain; never binds to a bare public suffix; the whole jar is still stored so cross-domain cookies ride along) and `adhoc-ceremony-capture.test.ts` (the httpOnly session-fixation rider is ignored on the push path).

- 2026-08-26 - D-CEREMONY-STREAM: the attended-ceremony login window is streamed live to the human's dashboard, so they can log in from whatever device they are on rather than only at the bridge machine. Closes the residual on `attended-ceremony-headed-browser-is-not-viable-for-a-normal-user` (the window opens on the bridge, which may not be where the human sits). MECHANISM: a JPEG CDP screencast of the ceremony window travels daemon -> Cortex as a new `ceremony.frame` BridgeFrame and is relayed down a dashboard media-channel socket; the human's mouse/keyboard travel up that socket and back to the daemon as `ceremony.input`, dispatched with CDP `Input.dispatch*`. `ceremony.stream{on}` starts/stops the screencast so a ceremony nobody is watching costs no frames. It REUSES the existing canvas media channel wholesale - the same short-TTL, single-use, `ekoa-canvas`-audience token (`streaming/auth.ts`), the same wire messages (`streaming/protocol.ts`), the same 1000/4000 close contract - but as a SEPARATE session type + path (`/api/v1/ceremony-stream/`, `streaming/ceremony-stream.ts`) because its frames are pushed IN from the bridge and its input is pushed OUT to the bridge, keyed by the ceremony `requestId` rather than a run traceId. GATED BY A NEW CAPABILITY `attended.livestream` (advertised by default by a daemon new enough to produce the screencast; an older one omits it and the ceremony stays a local-window flow) and by owner scope (the token subject must own the ceremony) + single-use token + a live registered session. CREDENTIAL TRANSIT (the load-bearing rule): during a ceremony the human types their real password into the streamed view, so a `ceremony.input` key event carries a password character and a frame is a picture of the login page. Both cross Cortex in RAM ONLY and are NEVER written to a log, a trace or the ledger - the same obligation `secret.deliver` carries (Cofre transit rule). The relay code logs by requestId only; `ceremony.input`/`ceremony.frame` payloads are never logged on either side; the frame rides `redactInboundFrame`'s default (untouched, it is an image) exactly as `tool.result.screenshotB64` does; the site masks the password field in the frame itself. WHY NOT the user's everyday browser (which would need no login at all): Chrome 136 blocks CDP on the default profile (the infostealer hole, see D-CEREMONY-REALCHROME), so the streamed window is the dedicated ceremony profile and the human still logs in once per site - but now from their own device. Tier boundary kept: `bridge/` does not import `streaming/`; the composition root (`server.ts`) injects `onCeremonyFrame`/`onCeremonyEnded` into `attachBridgeServer`. RESIDUAL, accepted this slice: the Cortex-side bridge WSS has no per-frame `maxPayload`, so a hostile paired daemon could send an oversized `ceremony.frame` (bounded by ws's 100MiB default). A tighter cap needs a size analysis of every bridge frame (session.push storageState, tool.result output) and is deferred rather than guessed at here.

- 2026-08-26 - D-CEREMONY-CHROME-SIGNIN: the ceremony card OFFERS, optionally, that the human sign into Chrome inside the ceremony window to reuse their saved passwords, with the tradeoff stated and an explicit opt-out (operator request). The ceremony runs in a real Chrome profile private to Ekoa on the user's machine (D-CEREMONY-REALCHROME); signing into Chrome (browser-level sync) there makes Chrome autofill the user's saved passwords, so they click through logins instead of typing. WHAT EKOA DOES AND DOES NOT SEE: nothing changes about custody - Ekoa never reads the Google account or the synced passwords (they live in Chrome's own OS-encrypted store); it only ever captures the resulting site session (cookies) for the origin the ceremony was about, and with the live stream a typed password transits Cortex RAM-only and unlogged (D-CEREMONY-STREAM). THE TRADEOFF, STATED TO THE USER: Chrome Sync pulls their password vault onto THIS machine's Ekoa profile - no different from their normal Chrome when the bridge is their own laptop, but a real exposure on a shared/remote box - so it is opt-in and declining just means typing passwords by hand. IMPLEMENTED AS GUIDANCE, NOT AN AUTOMATED FLOW: there is no reliable programmatic way to drive Chrome's browser-level sign-in (chrome:// is not navigable via CDP goto), so the card explains how (the Chrome menu, "Ativar sincronização") rather than a button. LIVE-VERIFICATION DEPENDENCY: whether Google permits Chrome sign-in / sync in this window at all is subject to Google's automation detection; the old bundled-chromium ceremony was refused (`google-sso-refuses-the-automated-ceremony-browser`), and the real-Chrome window (no `--enable-automation`, webdriver hidden, real channel) MAY now pass but is unverified - so the card's Google warning was softened to "if it does not advance, use email/phone" rather than an absolute block, and the residual is tracked in findings. NOT a security change: no new credential path, no new store; the session capture, its binding and its grant are exactly as before.

- 2026-08-26 - D-CEREMONY-STREAM-REVIEW: the adversarial pass on D-CEREMONY-STREAM (a new credential-transit path) cleared keystroke/frame privacy, the token/upgrade, and remote-input-into-the-window, and closed the two gaps it found - so the `maxPayload` residual named as "accepted this slice" in D-CEREMONY-STREAM is now FIXED, not deferred. (1) `pushCeremonyFrame` bound the viewer session by requestId alone; a compromised daemon on another pairing that learned the unguessable requestId could paint a spoofed frame onto the owner's dashboard. The frame path is now bound to the delivering pairing (`ceremony-stream.ts` records `pairingId` at `openCeremonyStream` and `pushCeremonyFrame(requestId, pairingId, ...)` drops a mismatch), mirroring the binding the input path and `acceptSessionPush` already had. (2) The daemon-facing bridge WSS had no `maxPayload`; `ceremony.frame` at 15fps turned the pre-existing 100MiB ceiling into a sustained vector. Capped at 32MiB (`MAX_BRIDGE_FRAME_BYTES` in `bridge/server.ts`) - above any legitimate frame (screencast JPEG ~150KB, viewport screenshot low-MB, storageState smaller) and a 3x cut. Remaining LOW residual: the stream token rides the URL query like the canvas channel it reuses (single-use + 600s TTL; moving it off-URL would diverge from the shared canvas client) - deferred, not a hole.

- 2026-08-26 - D-CEREMONY-STREAM-LIFECYCLE: an adversarial lifecycle+binding audit run BEFORE any live pass (ultracode verification) found five bugs the unit tests and the two security reviews missed - all in the LIFECYCLE and multi-domain flows, not the security properties - and all are fixed on branch fix/ceremony-stream-lifecycle. THE MOST IMPORTANT INSIGHT: the original Uber Eats failure was most likely NOT the binding and NOT purely the environment; it was that `acceptSessionPush` gated on an EXACT-HOST comparison of the ceremony origin against the daemon's LANDED URL (`sameOrigin`). A multi-domain login lands on the second domain (auth.uber.com) or the www-canonicalised host, so the capture was silently vetoed and the run parked in needs_credentials forever. That gate is removed; the real control is `boundOriginsForEstablishedHost(jar, ceremony.origin)` (the jar must cover the CEREMONY origin, which the login's .ubereats.com cookie does). The docblock had always conceded the field comparison "only ever pretended to be" the origin cross-check. The other four: (L1) the viewer's ceremony.stream{on:true} arrived while Chrome was still launching (common: ~200ms response vs 1-3s launch) and was dropped, never replayed -> black canvas forever - fixed with a buffered `wantStream` applied at onStreamReady; (L2) no viewer re-attach - a page reload killed the stream and re-clicking establish minted a second ceremony the daemon refuses - fixed with `findOpenCeremony` (owner+pairing+origin scoped) so establish reuses an open ceremony's requestId; (L3) the stream controller could not cancel an in-flight start(), orphaning a 15fps screencast with no viewer - fixed with a `desired` state re-checked after each await; (L4) a refused push never tore the stream down and the backstop was token-TTL-derived (a low TTL could cut a live stream) - fixed with onCeremonyEnded in the catch + a fixed CEREMONY_STREAM_MAX_MS backstop. Two narrower residuals are documented OPEN in findings, not fixed this pass: a direct-second-registrable-domain unattended target still needs a fresh ceremony (per-exact-host profiles + binding-keyed discovery; the fix wants a public-suffix helper), and an SPA fetch-login closed inside the 2s snapshot tick can mint a signed-out item (the Done button, the primary path, snapshots fresh and is unaffected). This is the adversarial-verification loop working as intended: the feature was unit-green and merged, but the audit caught real showstoppers before a live pass, and the fixes are proven by a new through-the-wire E2E integration test.

- 2026-08-27 - D-CEREMONY-STREAM-LIVE-FIXES: the FIRST live pass of the ceremony live-stream (D-CEREMONY-STREAM) exposed two defects that every unit test, security review and through-the-wire E2E had missed because they all used absolute test URLs and synthetic frame sizes. Both are fixed. (1) STREAM NEVER CONNECTED IN DEV (the socket sat at `connecting`, black canvas). Both streaming servers mint a RELATIVE `wsUrl` (`/api/v1/ceremony-stream/<id>`, `/api/v1/automation-stream/<id>`) because production is same-origin behind one edge proxy - but `new WebSocket(<relative>)` resolves against the PAGE origin, and in split-origin dev the page is the Next server (`:3000`) while the WS handler lives only on the API (`:4111`), so the socket dialed `:3000` and never reached the ceremony-stream upgrade handler. This means streaming had NEVER worked in any split-origin dev, only same-origin prod; the earlier B17 decode fix was necessary but not sufficient. FIX: `web/lib/api/canvas.ts` `toAbsoluteWsUrl` resolves a relative `wsUrl` against `resolveBaseUrl()` (the single API-origin resolver, which already adopts the browser host for LAN/Tailscale dev), swapping http->ws/https->wss; same-origin prod is unchanged (resolveBaseUrl returns the page origin there), split-origin dev now reaches the API. Regression tests in `canvas.test.ts` (relative -> API port; absolute passes through). (2) INPUT LANDED ON THE WRONG ELEMENT (clicking "Log in" opened the link beside it). Chrome screencasts at DEVICE resolution, so on a Retina window (deviceScaleFactor 2) the ceremony frame was 2x the CSS viewport, and its pixels no longer mapped 1:1 to the CSS coordinates `Input.dispatchMouseEvent` takes; compounding it, the viewer's display box was locked to a guessed 1280x800 aspect (`session.viewport`) that mismatched the real window, object-contain letterboxed the frame, and clicks were mapped over the black bars. FIX, two halves that must ship together: the bridge producer (`clients/bridge/src/attended/screencast.ts`) now reads the page CSS viewport via CDP `Page.getLayoutMetrics` and passes it as `maxWidth`/`maxHeight` to `Page.startScreencast` (frame collapses to 1:1 with the input coordinate space, exactly as the hosted canvas does with `page.viewportSize()`; also ~halves Retina bytes); and the viewer (`web/components/streaming/live-canvas-view.tsx`) sizes its display box to the ACTUAL frame aspect and maps clicks into the frame's natural pixel space, so there is no letterbox and the mapping is 1:1 for any real viewport. Unclamped fallback (a 1x display, or a CDP that does not answer getLayoutMetrics) stays correct because frame px == CSS px at deviceScaleFactor 1 and the viewer adapts regardless. The bridge half requires a client repack/reinstall to take effect. No structural/data-shape change (no new frames, modules or model): diagram invariant not triggered. Confirmed live: stream connects (green "Ligado"), frames paint; input-coordinate half pending the operator's repack + live retest.

- 2026-08-27 - D-CEREMONY-STREAM-LIVE-FIXES follow-up: the first repack of the coordinate fix streamed BLACK (green socket, no frames). Root cause was a gap the hosted twin never had: the ceremony screencast relied solely on `Page.screencastFrame`, which only fires on a repaint - an already-loaded headed login page emits no frame after the initial compositor tick - and the newly-added `Page.getLayoutMetrics` probe `await` could stall `Page.startScreencast` outright. FIX (`clients/bridge/src/attended/screencast.ts`): the producer now pushes an immediate `Page.captureScreenshot` frame after `startScreencast` and polls one every 500ms whenever the screencast has gone quiet (`POLL_INTERVAL_MS`, mirroring `api/src/streaming/session.ts`'s exact fallback), and the viewport probe is timeout-bounded (`VIEWPORT_PROBE_TIMEOUT_MS` 1500ms -> unclamped on timeout) so it can never hold up frame production. Screenshots are pictures of the login page and ride the same no-log `ceremony.frame` path as screencast frames; the poll timer is unref'd and cleared on stop. Tests: an immediate screenshot reaches the viewer with no screencast frame fired; the unclamped fallback still holds.

- 2026-08-27 - D-CEREMONY-STREAM-LIVE-FIXES second follow-up (coordinate space): the second live pass showed frames painting but clicks still landing on the element BESIDE the target. Root cause was that the coordinate space was tied to the FRAME's pixel dimensions, which are not stable: a HiDPI window screencasts at 2x the CSS viewport, and the screenshot-fallback frames (from `Page.captureScreenshot`, which honours the device scale factor) are a different size again than the clamped screencast frames - so whichever source painted last decided the scale, and on a static page the 2x screenshot won, mapping every click at 2x. FINAL FIX: decouple the coordinate space from the frame pixels entirely. The daemon now stamps each `ceremony.frame` with the window's CSS-pixel viewport (`cssWidth`/`cssHeight`, additive Rule-7 fields from `Page.getLayoutMetrics`); Cortex forwards it to the viewer as a `viewport` media-channel message whenever it changes (`ceremony-stream.ts` `pushFrame`); and the dashboard maps a click to a FRACTION of the rendered image times that reported CSS viewport (`live-canvas-view.tsx` maps to `viewportRef`, the reported viewport, not the frame's own pixels), while still sizing its display box to the frame's aspect (which equals the CSS-viewport aspect, so no letterbox). This is correct for any device scale factor and any frame source. The `Page.startScreencast` maxWidth/maxHeight clamp stays as a bandwidth win (Retina frames halved) but is no longer load-bearing for correctness. Touches shared (additive wire field, not an HTTP contract - no OpenAPI drift), api and bridge and web; both the VM API and the client bridge must be rebuilt to take effect. Tests: the daemon stamps every frame (screencast + screenshot) with the CSS viewport; Cortex emits a `viewport` message on first report and only re-emits on change.

- 2026-08-27 - D-CEREMONY-STREAM-ONE-TAB: the ceremony live view kept the whole flow in ONE streamed tab, fixing the "it takes over the computer" runaway and the invisible-login-tab. Live, clicking a portal's "Log in" opened a NEW tab (Uber Eats: "Sign up or Log in with Uber"), but the stream follows only the ceremony's own tab (`pages()[0]`) - so the login was invisible and uncontrollable in the dashboard, the human clicked again thinking it failed, and every new tab RAISED the headed Chrome window on macOS (D-CEREMONY-REALCHROME already conceded macOS raises the window per navigation). Repeated clicks x new tabs x the screenshot-poll force-compositing a now-background tab spiralled into the window stealing focus until the bridge was killed. FIX (`clients/bridge/src/browser/chrome-launch.ts` `SAME_TAB_INIT_SCRIPT`, added at launch): `window.open` navigates in place and a capture-phase click strips `target="_blank"`, so new-tab/popup opens stay in the streamed tab - the login is now visible and controllable, no tab pile-up, no per-tab window raises. Belt-and-braces: the screenshot fallback now captures with `fromSurface:true`/`captureBeyondViewport:false` so a poll never force-composites (raises) a tab. LIMITATION, documented: an OAuth login that genuinely needs a separate popup window posting back to its opener is not served (the card already steers Google-SSO users to email/phone); the portals this rail targets use full-page logins. Bridge-only change (client repack required). Tests: the same-tab init script is installed on the launched context.

- 2026-08-27 - D-CEREMONY-STREAM-COORDS-2: an adversarial multi-agent audit of the whole input-coordinate chain (17 agents, `verify-ceremony-coordinates` workflow) - run before another live repack - confirmed FOUR real defects; the numeric walkthrough proved the chain is otherwise correct (a clean click hits its target to ~1px). The likely LIVE Uber cause was #1. All fixed. (1) THE SCROLLBAR (producer): `cssViewport()` read `Page.getLayoutMetrics.cssLayoutViewport.clientWidth`, which EXCLUDES a space-reserving vertical scrollbar - but both the screencast surface and `Input.dispatchMouseEvent` use the FULL visual viewport (`window.innerWidth`, scrollbar column included). On a tall login page (Uber) the stamped width was ~15-30px short of the frame span, so every click near the right edge under-mapped left onto the neighbouring nav link ("Log in" -> "Get a ride"). FIX: read `window.innerWidth/innerHeight` via `Runtime.evaluate` (`clients/bridge/src/attended/screencast.ts`) - the exact space input is dispatched in - as both the screencast clamp and the stamped `cssWidth`/`cssHeight`. (2) LETTERBOX ASPECT SOURCE (viewer): `canvasToViewport` computed the object-contain letterbox from `viewportRef` aspect, but the on-screen geometry (canvas intrinsic size + CSS `aspectRatio`) is driven by `frameSize`; a mismatch fabricated phantom bars. FIX: `imgAR` now from `frameSizeRef` (`web/components/streaming/live-canvas-view.tsx`), self-consistent with the drawn bitmap - renderedW===boxW in the common case. (3) BORDER BOX (viewer): mapped over `getBoundingClientRect` (border box) while object-contain fits the CONTENT box; FIX: inset by `clientLeft`/`clientTop`, size from `clientWidth`/`clientHeight`. (4) STALE PLACEHOLDER ON RECONNECT (both ends): a reconnecting/second viewer stayed on the 1280x800 placeholder because `pushFrame` only emits `viewport` on CHANGE and a re-render re-clobbered `viewportRef` back to the placeholder. FIX: server re-sends the known `cssViewport` on `attachSocket` (`api/src/streaming/ceremony-stream.ts`); the client seeds `viewportRef` once per connection and lets `onViewport` own it thereafter (removed the re-seeding effect), and clears the canvas on reopen. Also, the `--no-sandbox` banner over the login (a real automation tell + security downgrade, Playwright defaults `chromiumSandbox:false`) is removed by `chromiumSandbox:true` on the real-Chrome launch (`chrome-launch.ts`). Bridge + web changes take effect on client repack + hot reload; the api reconnect fix (#4) lands on the next stack restart. Tests updated across all three layers.

- 2026-08-27 - D-CEREMONY-STREAM-HIDDEN: the ceremony window is now MINIMIZED off the desktop so it cannot take over the machine, while staying a real (visible-class, low-detection) headed Chrome. Operator decision (chose "headed but minimized" over headless): once the live stream is the control surface, the local window never needs to be on-screen, but a headless browser is more bot-detectable, so keep real headed Chrome and just hide it. WHY IT WAS TAKING OVER: a headed Chrome window on macOS raises to the foreground when it is created and on navigations (a login redirects through several pages: ubereats.com -> auth.uber.com -> back), covering the dashboard the human is driving from - and on the same machine (bridge == viewer) that is a fight for the desktop. FIX (`clients/bridge/src/browser/chrome-launch.ts`): after launch, minimize via CDP `Browser.getWindowForTarget` + `Browser.setWindowBounds{windowState:'minimized'}` (exposed as `context.minimizeWindow()`), and re-minimize on each navigation (`attended/ceremony.ts` framenavigated + after the initial goto), because macOS can restore a minimized window on some redirects. Anti-throttle launch flags (`--disable-backgrounding-occluded-windows`, `--disable-renderer-backgrounding`, `--disable-background-timer-throttling`) keep the hidden window RENDERING at full rate so the screencast does not freeze; the screenshot-poll fallback covers any gap. Best-effort throughout (a platform/CDP that cannot manage windows just leaves it visible). In production the bridge and viewer are separate machines, so this was strictly a same-machine annoyance, but the operator runs both on one box. RESIDUAL to watch live: whether macOS keeps the minimized window rendering (screencast alive) and whether any redirect still restores it past the re-minimize. Bridge-only change (client repack).
- 2026-08-27 — Token economics (ekoa-dev `docs/token-economics.md` port; full detail in this repo's `docs/token-economics.md`): **follow-up app/artifact builds no longer resume the Claude Agent SDK session.** Resuming (`agents/build.ts` `resume ← artifact.data.sdkSessionId`) regrew an unbounded transcript that never compacted under `claude-opus-5`'s 1M-default window and floored every follow-up to Opus — five prompts could burn a whole 5-hour usage window on ONE artifact, no other users present. Decision: every build runs a FRESH session; continuity is carried by the files on disk + a running ≤600-word `data.buildSummary` (fire-and-forget FAST/Haiku pass after each SUCCESSFUL build, `agents/build-summary.ts`, same shape as the §5.8 memory extractor) + a short verbatim conversation tail (6 msgs/24k with a summary, legacy 16/48k without) + an agent-maintained `NOTES.md`. Follow-up **routing floor lowered EXPERT→WORKHORSE** (`claude-sonnet-5`): a routine edit is a Sonnet task; a genuine rebuild self-escalates via 2+ Tier-4 keyword hits (classify() reads the RAW description, never the augmented prompt), and PT-PT big-change verbs / triviality markers were added to `llm/router.ts`. Accepted trade-off: detail older than the tail survives only via the inspectable+correctable summary or `NOTES.md` (compaction lost it opaquely too); the first follow-up after deploy has no summary yet and gets the legacy 16/48k tail (nothing lost during rollout). Auto-compaction is retained only as a backstop for one runaway build (`CLAUDE_CODE_AUTO_COMPACT_WINDOW=200000`, `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=60` on the build subprocess env), not the main lever. **Do NOT reintroduce `resume` on the build path** (even behind a flag): it silently recreates the whole problem as transcripts regrow. `data.sdkSessionId` is retired as a resume key but stays a reserved `data` key (legacy rows carry it). Diagram `04-agent-job` updated (FIXED-12); parity ledger + this journal record it.

- 2026-08-27 - D-CEREMONY-STREAM-HIDDEN follow-up: minimizing (previous entry) was WRONG on macOS - a minimized window stops compositing, so the screencast went black (confirmed live: the stream returned the moment the window was un-minimized). SUPERSEDED by moving the window OFF-SCREEN instead: it launches at `--window-position=-32000,-32000` and `attachWindowHide` re-applies `Browser.setWindowBounds{left:-32000,top:-32000,windowState:'normal'}` at launch and on every navigation. windowState stays 'normal', so the window keeps compositing at full rate (with the anti-throttle flags) and the screencast stays live, but the window sits outside every display so it never covers the desktop. Still best-effort and still bridge-only. REMAINING live risk: even off-screen, if a navigation makes Chrome the ACTIVE macOS application it could steal keyboard focus from the dashboard tab (a focus-only, not visual, takeover); if that shows up, the fallback is headless (which the operator declined for detection reasons). The `context.minimizeWindow()` seam name is kept for its intent (keep the window out of the human's way) though it now hides off-screen rather than minimizing.

- 2026-08-27 - D-CEREMONY-STREAM-HEADLESS-SWITCH: off-screen (previous entry) stopped the VISUAL takeover but not the FOCUS one - dispatching input into a headed window keeps making Chrome the active macOS app, stealing keyboard focus from the dashboard tab, so on a same-machine bridge the panel was barely typable (confirmed live: "the browser keeps gaining control so it's difficult to even type"). This is app-activation, not window position, so no headed trick fixes it on one machine. ADDED a switch: `EKOA_CEREMONY_HEADLESS=1` runs the ceremony windowless (`launchHeadedRealChrome({headless})` -> Playwright `headless`, skipping the off-screen args + `attachWindowHide`). Headless has no window/app to activate, so the human types into the streamed panel uninterrupted; screencast + input dispatch work exactly as headed. Headed-off-screen stays the DEFAULT because headless is more bot-detectable and because in PRODUCTION the bridge is a separate machine from the viewer and steals no focus - the operator flips headless on only for same-machine testing. `attended/ceremony.ts` reads the env once and threads it through both launch attempts. Bridge-only change. This closes the "takes over the machine" thread for the operator's single-box setup while leaving the low-detection headed path intact for real deployments.

- 2026-08-27 - D-CEREMONY-STREAM-VISIBLE: reverted the window-hiding machinery (minimize, then off-screen) - the ceremony window is a NORMAL VISIBLE headed Chrome again. THE FUNDAMENTAL CONSTRAINT, established the hard way across the day: on macOS a headed Chrome DRIVEN via CDP input keeps making itself the active application, stealing keyboard focus from the dashboard tab - and no flag, window position, or CDP call stops that (off-screen does not: it is app-activation, not window position; minimized freezes compositing -> black stream; headless works but Uber/most portals detect it - confirmed live, "Performing security verification"). So "same machine + type in the streamed panel + real visible browser + undetected" is impossible on one Mac. THE RESOLUTION: with a visible window the human logs in DIRECTLY in it (real keyboard/mouse, so it holds focus and nothing is stolen) and the panel is a live MIRROR; the panel-as-input-surface is for PRODUCTION where the bridge is a SEPARATE machine and cannot steal the viewer's focus. Kept: `EKOA_CEREMONY_HEADLESS=1` (D-CEREMONY-STREAM-HEADLESS-SWITCH) for same-machine panel-driven work where detection is acceptable (e.g. a less-hostile portal); the anti-throttle flags (so a visible-but-OCCLUDED window - behind the dashboard - keeps rendering the mirror); same-tab; all the coordinate fixes. Removed: `--window-position`, `attachWindowHide`/`minimizeWindow`, the per-navigation re-hide (which was itself activating Chrome and amplifying the focus theft), the OFFSCREEN consts. This restores the D-CEREMONY-REALCHROME visible window, now without the tab-flap (same-tab), the automation banner (sandbox + --enable-automation drop) or the coordinate errors. A future macOS-only "visible but background" launch (`open -g` + connectOverCDP) could let the panel drive a visible window without focus theft, but is a larger re-architecture and unverified - deferred. Bridge-only change.

- 2026-08-27 - D-CEREMONY-STREAM-POLL-HEADLESS-ONLY: found the real cause of the "aggressive loop, can't even kill the bridge" on a headed same-machine ceremony - the recurring screenshot poll (`clients/bridge/src/attended/screencast.ts`, added for the black-canvas fallback). On a HEADED macOS window, `Page.captureScreenshot` on an OCCLUDED window (the operator clicked away to the dashboard/terminal) forces the window back to the FRONT to grab its surface; a 500ms poll yanks Chrome forward twice a second, so the operator cannot hold focus on anything else - including the terminal to Ctrl-C. FIX: the recurring poll now runs ONLY in headless (`POLL_WHEN_IDLE = EKOA_CEREMONY_HEADLESS==='1'`), where there is no window to raise and the screencast is genuinely silent on a static page. A headed window relies on the screencast alone, which reads the compositor with the anti-throttle flags and never raises. The immediate FIRST screenshot still runs in both modes (the window is frontmost at launch, so it fights nothing). This should make the visible-window direct-login flow calm (the window raises only on its own navigations, not on a timer). Bridge-only change.
