...
S9 adds one more, and it is ABSENT-MEANS-REFUSE rather than absent-means-skip: `readTenantDataset`
(`integrations/` -> `legal/`), bound to `legalTenantReadHandler`. It is the execution seam for the
`tenant-read` backing below. Absent, a tenant-read action is refused `unsupported_backing_type` -
the honest answer, because the alternative (an empty list) reads to a lawyer as "you have no
processes". It is threaded to ALL FOUR executor call sites (the automation seam's own executor, the
schedule supervisor, the capability rail and `achieve`); a seam bound at three of four is one rail
quietly behaving differently, which is what the note beside `runAutomationBackedAction` in
`server.ts` was written about.

### Action backings (`resolveBackingType`, `integrations/definitions.ts`)

Four, and the resolver is the ONE derivation every caller asks: `api-call` (an `httpConfig`),
`browser-steps` (an `automationBinding`, which wins over any `httpConfig` beside it), `bash-cli`
(runnable only once materialised as an automation - bash never runs on the Cortex host), and
`tenant-read` (S9). A `tenant-read` action names a `tenantRead.dataset` and CONTACTS NOTHING: it is
answered from data this platform already holds for the asking tenant, through a rail that did the
contacting earlier under its own verification. The dataset string is a key into a CLOSED set of
readers, never a query - an unbound one is refused `unknown_dataset`.

The property the backing enforces: **a tenant-read action never causes a credential to be
decrypted.** Its dispatch sits ABOVE `decryptCredentialFields`, so the decrypt, the WS-C shadow
comparator, the provider credential resolver and the egress binding are unreached rather than
unused. Everything above that line still applies - the write gate answers first, and a disconnected
or disabled integration still refuses. The one shipped instance is the `citius processos` action
over `legal/citius-processos.ts`; it records no evidence row, because an evidence sample is a record
of what an action did against a THIRD PARTY and this one contacted none (decisions.md D-S9-3).
