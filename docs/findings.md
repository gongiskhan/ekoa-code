...
## F-2026-07-18-invalid-date-cards (closed 2026-07-19, fixed on the parallel line)
Artifact cards on /artifacts rendered "Invalid Date" for dev-seeded artifacts whose createdAt is
empty (visible in the OS-mode walkthrough, classic beats). Surfaced by the walkthrough vision pass
on the OS-mode line; independently fixed on the parallel run-20260717-202309 line (S0) before the
two lines merged: `formatDate` guards invalid dates, date rows hide instead of rendering the
placeholder, and sorting falls back via `artifactSortTime`. At the 2026-07-19 merge the fix was
