/**
 * Persistence wiring for app-auth-persistent.
 *
...
// The app-data API wraps every payload in `{ success, data }` (see
// cortex/src/server.ts). Unwrap to `data` so callers get the raw record(s),
// matching the injected `window.__ekoa.list/get/create/update/delete` helpers.
