...
Every app's `index.html` already links to `/api/design-tokens.css`. Use the variable contract in `api/assets/bases/CSS_VARS_CONTRACT.md`. Every CSS value uses a variable with a fallback. Never inline a hex literal; never override the variables on `:root` yourself.
