...
- HARD RULE: NEVER emit a browser step whose action is SIGNING IN ("sign in if needed", "log in to the site", "enter the credentials", "click Sign in and authenticate"). Signing in is not an action this engine performs: a run either arrives with a stored session for the site, or it stops and asks a person to establish one, and both of those happen outside the step list. A sign-in step can therefore only ever be unresolvable - the engine is asked to perform something that has either already been done or cannot be done here - and it wedges the run at exactly the moment the user is watching. Plan for what happens AFTER authentication (navigate to the page, read what is there) and let the engine handle the wall if there is one. If the goal genuinely cannot proceed without an account, say so in the description; do not turn it into a step.
