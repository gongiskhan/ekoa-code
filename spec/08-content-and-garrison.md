...
| 2 | Org identity and branding | the org record (dynamic; per-org, ch04) | yes | yes | no |
...
- **Knowledge grounding is org-partitioned (Amendment 2).** The knowledge base - the vault and its lexical index - is partitioned by org: the firm's documents never pool across orgs, and the slot-5 grounding builder consumes only the caller's org partition. This is a scoping property of the knowledge surfaces and storage (ch03/ch04 own both); content packages carry no live knowledge data (slot 5 is a dynamic, code-built block, not content). Founder, 2026-07-06 (amendment 2, consolidated ledger, docs/ekoa-code-spec-amendment-2-consolidated-ledger.md).
...
| 13 | `cortex/apps/{ekoa.company, ekoa.deployments, ekoa.projects}` | The retired layer's three bundled definition directories, including their runtime write-back into the repo tree | **Dies with the machinery** (Tier C). Company data is superseded by the org record (ch04; Amendment 2 - the old `company` singleton is archived at migration and one org is seeded per existing user, not imported as a singleton); the write-back durability hole (same class as row 3 - reference/data-inventory.md §8) disappears with the layer |
...
*Amended again 2026-07-06 per the consolidated-ledger amendment (Amendment 2, docs/ekoa-code-spec-amendment-2-consolidated-ledger.md): stated that the knowledge base (vault + lexical index) is org-partitioned so the firm's documents never pool across orgs and the slot-5 grounding builder consumes only the caller's org partition (§8.4); renamed system-prompt slot 2 from company identity/branding to org identity and branding from the per-org record (ch04); swept the §8.5 Garrison-line worked example from per-tenant deny-lists and tenant configuration to per-org deny-lists and org configuration; and updated the row-13 fate so the old `company` singleton is superseded by the org record (archived at migration, one org seeded per existing user) rather than migrated as a singleton.*
