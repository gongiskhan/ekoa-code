...
- **Referenciar ficheiro/pasta local (Reference)** - opens an **in-app file browser** served by the daemon's loopback surface (`GET /browse`); the user navigates their own machine and picks a file or folder **visually** - no typed path, no typed identifier. The pick becomes a session grant (chapter 18 section 18.2) and a visible reference token in the composer. Reference never copies and never uploads.
...
**Amendment (2026-07-11, owner-directed).** FC-401's connected state originally opened the daemon's *native OS picker* (with a typed-identifier fallback while the picker was unbuilt). After the consumer-run walkthrough the owner directed that non-technical users must never type a path or a grantRef and never re-authorize per attach: **connected = trusted**, and selection is done through an **in-app file browser** (the daemon `GET /browse` surface, C1-C3 of docs/bridge-counterpart-changes.md, now implemented in ekoa-bridge). The native-picker `POST /picker` (former C4) and the typed-reference fallback are **retired**. Consent is unchanged: the first grant of a session still shows the FC-411 dialog once. Grants are minted at message-send time, bound to the hosted chat session id (so a chat delegation resolves them without CLI `--session` gymnastics); a file pick grants its parent folder, stated in the consent copy.

**Amendment (2026-07-11, owner-directed — attach clarity + not-connected routing).** Three UX reinforcements, no mechanism or claim change: (a) the Reference block in the attach menu (FC-400) is given a distinct **teal "safe zone"** treatment with a `ShieldCheck` icon and a **"Recomendado para documentos sensíveis"** badge (a verbatim substring of the FC-400 micro-copy, so no new claim), and the Upload group carries the caution caption **"Guarda uma cópia nos nossos servidores."** — the colour contrast makes the bridge the obvious path for confidential files. (b) FC-401's *offline* state now offers a primary **"Abrir definições da ponte"** CTA into the privacy settings surface (FC-404/FC-405) alongside the retry hint, so a not-connected user always has a one-tap route to the bridge page (it previously offered only retry). (c) The *not-installed* install CTA already routed there and is unchanged.
...
| FC-401 | Reference action states | build | Three states, driven by the bridge presence heartbeat (chapter 18 section 18.3): (1) **no bridge installed** - the action renders disabled with a short explanation and an install CTA linking to the daemon download/pairing flow; install-CTA primary copy (verbatim, v2 A7.2): "Os documentos dos seus clientes ficam no seu computador; o agente lê apenas o que precisa e cada leitura fica registada." with a "saiba mais" link into the settings privacy panel (FC-404). (2) **installed but offline** - "ponte offline" state with a retry hint; nothing is silently degraded to upload (chapter 18 section 18.2). (3) **connected** - the **in-app file browser** opens (daemon `GET /browse`); the user picks a file/folder visually and the pick becomes a session grant (minted at send, bound to the chat session) and a composer reference token. (2026-07-11 amendment: was the native OS picker + typed fallback.) |
...
| FC-405 | Bridge status and pairing | build | Bridge presence/heartbeat status and the pairing flow (chapter 18 section 18.3); the revoke-pairing kill switch is surfaced here. **Install/download area (2026-07-11 owner amendment, non-technical):** the bridge page leads with a "Descarregar e instalar a ponte local" section, above the status/pairing card. Primary path = **double-click installers**, no terminal: an OS toggle (Mac/Windows, defaulting to the visitor's platform) offering `Instalar-Ponte-Ekoa-Mac.zip` (a zipped `.command` that installs the CLI, runs device-login pairing, and starts `serve` via native `osascript` dialogs — the user only clicks and confirms the pairing code in the browser) and `Instalar-Ponte-Ekoa-Windows.bat` (runs the hosted `install.ps1`, same flow via `System.Windows.Forms` message boxes). Per-OS one-time security note (Gatekeeper right-click→Open / SmartScreen "Run anyway") stated honestly. A collapsible "Instalação avançada" holds the terminal path (`curl … install.sh \| bash` + tarball) for technical users. All artifacts are published to the public GCS bucket `ekoa-bridge-downloads`; URLs default in `web/lib/privacy-claims.ts`, overridable via `NEXT_PUBLIC_BRIDGE_MAC_URL` / `NEXT_PUBLIC_BRIDGE_WIN_URL` / `NEXT_PUBLIC_BRIDGE_DOWNLOAD_URL`. Note: the installers bake a Cortex URL (`EKOA_CORTEX_URL`, default `http://localhost:4111` for local testing) that must be set to the production app URL before real distribution |
