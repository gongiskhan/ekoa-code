...
Nine blocking, seventeen deferrable - twenty-six pre-amendment entries, all resolved as of 2026-07-06 (founder resolutions, amendment brief docs/ekoa-code-spec-amendment-brief.md). This amendment mints one further deferrable entry, P-27 (executor-face run-record retention), for twenty-seven total. P-27 is the only entry still reading pending, and it is deferrable with a safe default (detector-at-persist) that gate G-P stamps into the register at launch if the founder leaves it unmarked.
...
| P-08 | Supabase control plane | ch04 4.5 | **blocking** | Keep Supabase, reduced to the 3 live tables | ACCEPT |
...
| P-12 | Memory system scope in v1 | ch05 5.8 | **blocking** | CRUD + resolver injection on; auto-extract off by default | ACCEPT |
...
| P-27 | Executor-face run-record retention | ch17 17.10 | deferrable | Content-bearing run-record fields pass through the detector at persist time | pending (deferrable default) |
...
Minted 2026-07-06 by the anonymisation/local-file-access amendment (docs/ekoa-code-spec-amendment-brief.md, Part 3). It is the only entry in this register still open, and it is deferrable with a safe default; its inline `PROPOSED P-27` marker is the single live PROPOSED marker anywhere in the spec until gate G-P stamps the default.
...
- **Resolution:** pending (minted 2026-07-06 by this amendment; deferrable, resolves by default to the recommendation - detector-at-persist - at gate G-P if the founder leaves it unmarked).
...
4. **Blocking census:** exactly 9 entries were flagged blocking (P-01, P-03, P-05, P-08, P-10, P-12, P-15, P-18, P-25), and the nine blocking entries are all resolved as of 2026-07-06; chapter 14 gate G-P consumes exactly this list. The amendment adds no blocking entry - P-27 is deferrable.
5. **Launch gate:** every pre-amendment entry's Resolution column reads a recorded decision (ACCEPT, ALTERNATIVE, OTHER, or defaulted-to-recommendation for deferrable items) as of 2026-07-06; the single remaining "pending" entry is P-27, deferrable, whose safe default (detector-at-persist) gate G-P stamps into the register at launch if unmarked - so zero entries read "pending" once G-P runs. G-P halts only if a blocking entry lacks a resolution (chapter 14 section 14.2.4); none does.
6. **Patch discipline:** for every resolved entry except P-16, the owning chapter's inline marker reads `RESOLVED (P-nn)` and the decided text is normative prose (P-16 resolves in this register alone - it has no chapter marker by construction, section 15.4); cross-reference occurrences in other chapters flip to `RESOLVED (P-nn)` in the same pass (15.1 step 2). A spec-wide grep for `PROPOSED P-nn` returns exactly one hit - `PROPOSED P-27` in chapter 17 section 17.10 - until its default stamps at gate G-P, and zero hits in chapters 02-14 at launch.
