...
| Q-02 | Literal production app-data backend value (lives in ekoa-deploy, unverified) | Ops check | **blocks cutover** | spec assumes Firestore | Cutover-class; factual half closed (Amendment 2): backend value `fs` as-deployed, read from ekoa-deploy, recorded in RUN_LOG.md; ch10 §10.2 row 1 now defaults to the app-data import branch (Firestore no-migration repoint the cheaper branch), migration-time re-confirmation stays on the ch10 checklist |
...
**Resolution:** Unchanged - confirmed as a cutover-checklist ops action; answered at cutover, not at run start. It stays cutover criterion 6 (chapter 10 section 10.5); the implementation run does not block on it. Confirmed by the founder, 2026-07-06 (amendment brief, docs/ekoa-code-spec-amendment-brief.md). Downgraded further 2026-07-06 (amendment 2, consolidated ledger, docs/ekoa-code-spec-amendment-2-consolidated-ledger.md): the factual half is now closed - the production app-data backend value is `fs` as-deployed (read from `ekoa-deploy`, recorded in RUN_LOG.md at the repo root; the old docs' prod=Firestore claim traced to a hand-set `.env` the committed pipeline does not reproduce). Consequence: chapter 10's migration plan now defaults to the real app-data import branch (chapter 10 section 10.2 row 1 as rewritten, driven by the existing chapter 04 section 4.8 import tooling), with the no-migration Firestore repoint kept as the cheaper branch should a live-container re-confirmation prove production hand-set to Firestore; that re-confirmation stays on the chapter 10 migration-time checklist.
...
| No server-side token kill switch (invisible-behaviors 6) | Routed to P-03 (chapter 15; chapters 03 and 09; section 16.4 above) |
...
| License tier feature map computed but never enforced (invisible-behaviors 7) | Carried as-is, recorded honestly; runtime enforcement would be a new product decision, out of scope (chapter 09 section on license facts). Superseded 2026-07-06 (Amendment 2, Part 3): tiers and the tier feature map are dropped as a concept; access is the activation model (chapter 09) plus the billing gate |
| Orphaned `running` jobs after a crash (invisible-behaviors 14) | P-10 (chapter 15): minimal persistent job registry with boot-time orphan sweep (chapter 05) |
| Unbounded growth across stores (invisible-behaviors 22; data-inventory C12) | the P-09 retention table (chapter 15), with the founder's keep-forever call on automation runs preserved (chapter 04 section on retention) |
...
| Dead Supabase tables and license-key format drift (data-inventory C7) | P-08 originally kept only the three live tables; dead schema dropped (chapter 04). P-08 re-resolved (Amendment 2): Supabase retired entirely; chapter 10 owns the migration/retirement rows |
...
*Amended again 2026-07-06 per the consolidated-ledger amendment (Amendment 2, docs/ekoa-code-spec-amendment-2-consolidated-ledger.md): Q-02's factual half is closed - the production app-data backend value was read from ekoa-deploy during the amendment session and recorded in RUN_LOG.md, leaving a migration-time re-confirmation on the chapter 10 checklist; the section 16.5 rows for the license tier feature map and the dead Supabase tables gained cross-references to Amendment 2 (tiers dropped for the activation model; Supabase retired entirely, chapter 10 owning the migration/retirement rows). No question was renumbered or added.*
