...
| Model-provider credential + org/activation state | Firestore collections (same cluster) | P-08 re-resolved (Amendment 2) | 4.5 |
| Secrets at rest | AES-256-GCM ciphertexts, one crypto module, mandatory key | FIXED-8 + RESOLVED (P-14) | 4.7 |

Firestore here means Firestore Enterprise edition with MongoDB compatibility, reached over the MongoDB wire protocol via the `mongodb` npm driver. The rebuild standardizes on it as the single app-data backend: there is no Firebase/GCP SDK anywhere in the current backend, auth is a scoped connection-string database user, and index creation is an out-of-band admin script because the least-privilege runtime user cannot run admin commands (reference/data-inventory.md §1, §3.4). The rebuild keeps all of that and drops the dual-backend switch entirely (section 4.2.8). The production app-data value that selects the backend was read from the committed ekoa-deploy pipeline during the Amendment 2 session and is the **filesystem** backend (`EKOA_APP_DATA_BACKEND` unset, default `fs`), recorded in RUN_LOG.md; that closes the factual half of conflict C1 / **Q-02** (section 4.9), makes the filesystem-to-Firestore import the default migration path (section 4.8), and leaves a live-container re-confirmation on the chapter 10 checklist.
...
### 4.2.6 Engine scoping

- **No org dimension in the engine** (Amendment 2): the platform tenancy unit is now the org (chapter 09; Amendment 2 Part 4, replacing the single-company model of FIXED-8), but the collections engine keeps its `(appId, collection)` scoping unchanged - the org model adds no dimension to the engine. Apps are user-owned; org-shared visibility is a platform-domain concern (section 4.3.1), never an engine scope key.
...
8. **Parity tests**: one test suite, executed against every driver/target the engine supports (§3.7 migration note). In the rebuild the engine has one driver (Mongo wire protocol) and two targets: in-memory MongoDB (`mongodb-memory-server`) for tests and local dev, Firestore Enterprise in production. The suite runs against in-memory Mongo in CI on every build, and once against the real Firestore database as a pre-cutover smoke gate (chapter 10). The dual-backend switch is **not carried** (Amendment 2, Part 7.18): neither the filesystem driver nor its selector `getAppDataBackend()`/`EKOA_APP_DATA_BACKEND` exists in the new repo - Firestore Mongo-compat is the only app-data backend (P-05 makes it the dev store anyway). `mongodb-memory-server` is retained for dev and tests: it is the same `mongodb` wire driver pointed at an in-memory server and is unrelated to the removed filesystem backend, so it must not be over-deleted in the sweep. The narrow driver contract is kept so a second driver remains cheap if ever needed.
...
|---|---|---|---|---|
| `users.json` | `users` | `_id` = user id; username, bcrypt passwordHash, `role` (`super-admin` \| `org-admin` \| `builder`), required `orgId`, `active` (activation boolean, Part 3; carries today's `isActive`), preferences | `orgId` (Amendment 2); the user table itself | Preserve `preferences.approvedLocalCommandShapes` exactly, including the argv shape-normalization contract (security-relevant consent state - §5.1 users). Amendment 2 (Part 4): every user carries a required `orgId` at creation (creating a standalone user with an omitted `orgId` auto-creates an org with that user as its `org-admin`, chapter 03/09); `active` is the admin-controlled activation fact - deactivation, in the same write, pushes the user's tokens into the P-03 revocation set and updates the write-through activation map (chapter 03 `PATCH /users/:id`); the founder's account is seeded `super-admin` at migration (section 4.8 item 12) |
...
| (new; Amendment 2 org model - supersedes `company.json`, which is archived) | `orgs` | `_id` = org id; name, displayName, branding record (incl. brand-research result), settings, createdAt | the org table itself (platform tenancy unit) | Amendment 2 (Part 4): replaces the old `company` singleton. Schema/API name `org`; PT-PT display label "Escritório". The old `company.json` is **archived at migration, not imported** - migration instead creates one org per existing user, all on the default design system, no brand carry (section 4.8 item 12); the founder handles Brasil Salomão's brand re-setup post-launch. The prior every-boot grayscale rewrite is not carried |
| (was Supabase `standalone_credentials`; Amendment 2) | `credentials` | singleton `_id` = `default`; the environment's model-provider credential (ciphertext), `mode` (`oauth` \| `api-key`), refresh/expiry metadata | global (one credential per environment) | Replaces the Supabase `standalone_credentials` table (P-08 re-resolved, section 4.5). Ciphertext under the one crypto module (section 4.7); custody semantics (two auth modes as per-environment config, proactive refresh, refresh-and-retry-once on 401, alert on persistent failure) owned by chapter 06. The old multi-installation, per-installation-row, pool/rotation model is superseded (section 4.5) |
| `memories.json` | `memories` | `_id` = memory id; title, content, type, tags, tier, origin, score, `visibility` (`private` \| `org`), userId?, orgId, attachments | `visibility` (`private` \| `org`) + optional `userId` owner + `orgId`; automation scoping via `automation:<id>` tags | Dual role carried knowingly: human memories AND the automation action cache ride `attachments.payload` (§5.1 memory). `attachments.blobRefs` paths become storage-relative references (P-07). Amendment 2 (Part 4): `visibility` is `private \| org` - `private` is owner-only (invisible to org admins; existence appears in Registo metadata, never content), `org` is org-shared, and the resolver injects the user's own plus org-shared memories; automatic extraction (P-12) always writes `private` (sharedness is never inferred). Memory feature scope is P-12 (chapter 05) |
| `artifact-instances.json` | `artifacts` | `_id` = artifact id; typeId, name, userId, orgId, `visibility` (`private` \| `org`), status, slug, featured, health, data | `userId` owner + `orgId`; `visibility` (`private` \| `org`); `featured` surfaces regardless of owner | Slug uniqueness via the `slugs` reservation collection (below). Featured working-copy lens inside `data` carried (§5.1 artifact-instances). Amendment 2 (Part 4): `visibility` is `private \| org` (default `private`) - org-shared artifacts are visible **and editable** by org members (safe because git version snapshots + restore + Registo cover every mutation); private artifacts are owner-only, invisible to org admins (existence appears in Registo metadata, never content) |
...
| `integration-configs.json` | `integration_configs` | `_id` = config id; name, type, config, credentials (ciphertext), platformProvider, needsReauth, ownerUserId?, orgId, sessionState (ciphertext) | Amendment 2 (Part 4): **`ownerUserId` undefined means org-scoped/org-admin-authored (shared to the org); else owner-only** - the old global sense is re-scoped to the org (the Q-10 M365 workspace token is per-org) | Ciphertexts carried byte-compatible (section 4.7); `sessionState` never returned to any client |
| `activity-logs.json` | `activity_logs` | `_id` = log id; userId, username, orgId, category, type, timestamp | per-user rows, `orgId`-scoped; org-admin readable within its org, super-admin across orgs | Single write path (FIXED-8, Registo-ready); the username-stores-id bug is fixed as part of the port (chapter 09). Amendment 2: `orgId` added to back the Registo read surface (chapter 03 §3.8.24, Part 5). Index `{timestamp}`, `{orgId, timestamp}` |
...
| `teams.json` | **DROPPED** | - | - | Teams are deleted end to end by Amendment 2 (Part 4.14): endpoints, pages, stores, tests, collection, and migration rows all removed; not imported (chapter 10 section 10.8). Glossary row in chapter 11 records the retirement (departments, if ever demanded, return as additive groups inside an org) |
...
| `sandboxes/user-<id>/settings.json` (hidden per-user overrides) | `user_settings` | `_id` = user id; per-user settings incl. `build.verifyBuilds` (default ON, Part 6) and `memory.autoExtract` (default ON, P-12) | per-user | Surfaced as first-class data, out of the sandbox tree (§5.1 settings migration note). Amendment 2: carries the two per-user toggles - per-build verification (`build.verifyBuilds`, set once via the first-build ask-once dialog, chapter 12) and automatic memory extraction (`memory.autoExtract`, P-12/chapter 05) |
...
| `automations.json` | `automations` | `_id` = automation id; name, steps[], trigger, ownerUserId, orgId | `ownerUserId` owner + `orgId` (org-scoped: visible to owner + org admin, Part 4.9) | Step type union is the product contract, carried verbatim (§5.1 automations). Amendment 2: `orgId` is a **deliberate denormalization** - stamped once at creation from the owner's org so the Registo/org-admin surface reads it directly instead of joining through `users`; it is write-once (a user never changes org in v1; if org reassignment ever ships it is a migration). Index `{orgId}` |
| `automation-runs/<aid>/<rid>/run.json` | `automation_runs` | `_id` = run id; automationId, status, steps[], triggeredBy, orgId, pause/consent requests | via parent automation's owner + `orgId` (owner + org admin) | Step screenshots stay filesystem blobs (P-07), referenced by storage-relative path. Grows-forever is an explicit product decision carried into P-09 (conflict C12). Amendment 2: `orgId` denormalized from the parent automation at creation (write-once, as `automations`). Index `{automationId, startedAt}`, `{orgId, startedAt}` |
| `triggers.json` | `triggers` | `_id` = trigger id (opaque UUID, doubles as the public hook URL segment); ownerUserId, orgId, target discriminator, kind, integrationKey, secretCiphertext, disabled | `ownerUserId` server-trusted, never derived from a request (carried - §5.1 triggers) + `orgId` (owner + org admin) | The store's lifecycle change bus stays an in-process event emitter at the service layer - sound because single-process is FIXED-8; no Firestore listeners needed. Amendment 2: `orgId` denormalized from the owner at creation (write-once, as `automations`). Index `{orgId}` |
...
| `knowledge/sources.json` | `knowledge_sources` | `_id` = source id; url, kind, crawl config, seedId, orgId | **org-scoped** (org-admin-managed) - Amendment 2 (Part 4): the knowledge base is org-partitioned, the firm's documents never pool across orgs (section 4.4.1) | Seed idempotency via `seedId` lookup carried; the check-then-act seed lock becomes insert-with-deterministic-id |
| `knowledge/uploads.json` | `knowledge_uploads` | `_id` = upload id; filename, collection, docIds[], status, orgId | **org-scoped** - Amendment 2 (Part 4): org-partitioned knowledge base (section 4.4.1) | `storedPath` is an absolute path today - becomes a storage-relative reference (carried requirement - §5.2 uploads) |
...
| Legacy content-layer data files (`<dataDir>` per-app JSON, §7.1 row 1) | **DROPPED** (the old `company.json` is archived, not imported - superseded by `orgs`, above) | - | - | The layer that wrote them does not exist in the rebuild (FIXED-4) |
...
| `users` | `{orgId}` | Org user lists (Registo, org-admin users page - Amendment 2) |
...
| `activity_logs` | `{timestamp}`, `{userId, timestamp}`, `{orgId, timestamp}` | Audit reads, retention sweep, Registo org surface (Amendment 2) |
| `automations` | `{orgId}` | Org-admin automation listing (Amendment 2) |
| `automation_runs` | `{automationId, startedAt}`, `{orgId, startedAt}` | Run history (replaces mtime-desc directory listing); org-admin/Registo run listing (Amendment 2) |
| `jobs` | `{status}`, `{userId, createdAt}` | P-10 orphan sweep, per-user job lists |
| `artifacts` | `{userId}`, `{featured}`, `{orgId}` | Gallery and starting-points queries; org-shared listing (Amendment 2) |
| `memories` | `{tier}`, `{tags}`, `{orgId}` | Resolver candidate narrowing (scoring stays in code); org-shared injection (Amendment 2) |
| `app_sessions` | `{expiresAt}` | Expiry sweep |
| `triggers` | `{ownerUserId}`, `{kind}`, `{orgId}` | Owner lists, listener supervision; org-admin listing (Amendment 2) |
| `revoked_tokens` | `{expiresAt}` | Boot-load scan + retention self-prune (RESOLVED (P-03); section 4.3.4) |
| `anonymisation_deny_lists` | `{orgId}` | Deny-list load per org (chapter 17.4; section 4.3.4) |
| `bridge_pairings` | `{orgId}`, `{userId}` | Pairing lookup and revoke (chapter 18.3; section 4.3.4) |
...
|---|---|---|---|---|
| Per-org anonymisation deny-lists | `anonymisation_deny_lists` | `_id` = entry id; orgId, value (ciphertext), entityClass, addedBy, addedAt | org-scoped | Secret-material: entries are encrypted at rest with an org-scoped key through the one crypto module (section 4.7), never returned in cleartext to any client, and every read is access-logged to the Registo write path. Chapter 17.4 owns detection semantics |
| Bridge pairing registry | `bridge_pairings` | `_id` = pairing id; orgId, userId, createdAt, revoked (boolean) | org-scoped, `userId`-owned | Small collection; the `revoked` flag is the server-side kill switch - a revoked row can never re-authorize a daemon connection. Chapter 18.3 owns pairing semantics |
...
- **Org-partitioned** (Amendment 2, Part 4): the vault and its FTS index are partitioned per org - each org's markdown corpus lives under its own path segment and its FTS rows carry the org partition, so a firm's documents never pool across orgs and no cross-org search is possible. The knowledge registries (`knowledge_sources`, `knowledge_uploads`, section 4.3.1) carry `orgId` to match.
...
## 4.5 Control plane - RESOLVED (P-08), re-resolved (Amendment 2)

**RESOLVED (P-08), re-resolved (Amendment 2): Supabase is retired entirely.** The control plane no longer runs on Supabase. Its three live tables move into Firestore collections on the same cluster and driver as everything else in this chapter, and the abandoned dead-table schema drops with the project (section 4.8; chapter 10 owns the cutover procedure and the project's retirement). The previous resolution kept Supabase as the control plane, reduced to three tables; Amendment 2 (Part 1) supersedes it - one less external system, and the credential/org/activation state joins the product data plane under the same crypto module, backup story, and erasure workflow.

**(a) `credentials` replaces `standalone_credentials`.** The environment's model-provider credential moves to the `credentials` Firestore singleton (`_id: 'default'`, section 4.3.1): the ciphertext credential plus a `mode` field (`oauth` | `api-key`) that selects the auth mode as per-environment configuration (FIXED-8 as amended, Part 1). Chapter 06 owns custody semantics; the shape here is simplified per Part 1.2 - **one credential per environment**; proactive refresh before expiry in `oauth` mode; refresh-and-retry-once on 401; alert on persistent failure (surfaced on `/health`). The ciphertext rides the one crypto module (section 4.7).

The eight load-bearing OAuth rotation semantics of the prior resolution (persist-first-then-commit, single-writer refresh mutex, jittered proactive refresh, the 20-minute four-mode watchdog including peer-installation token adoption, keep-row-on-401, break-glass env fallback, subprocess-env sync, and per-installation boot gating) and the multi-installation model they served are **superseded and not carried**. They encoded the operational incidents of the retired multi-installation Supabase design - multiple installations rotating one shared credential row - so with one credential per environment and no installation rows there is nothing left for that machinery to coordinate. What remains is the simplified custody above: no credential pools, no rotation, no health-scoring, no selection logic.

**(b) `companies` is superseded by the org model.** License lookup by company is gone; the platform tenancy unit is the org (`orgs` collection, section 4.3.1; Part 4). No company row, no tier-to-feature map - tiers are dropped as a concept (Part 3).

**(c) `installations` and the license facts fold into the activation model.** There are no installation rows, no license key, and no tiers. Access is governed by the activation model of chapter 09 §9.7.1 (the `active` boolean plus the existing billing gate); "license" reduces to activation + billing.

**No license server exists.** Verified against ekoa-dev during the Amendment 2 session (recorded in RUN_LOG.md): there is no separate license-server application anywhere in ekoa-dev. License logic lived entirely inside the old Cortex as a boot gate (two Supabase lookups - `companies` by license key must be active; `installations` by installation id must exist, belong to that company, and be active - plus a fire-and-forget `last_seen_at` heartbeat), a tier feature map that was computed and never enforced, and two registry-registered tools that no handler dispatched; the frontend consumed no license surface. Nothing folds into `api/` beyond the activation model itself (chapter 09) and the credential custody above (chapter 06). The old boot gate and the entire license plane retire with the old stack.

Rejected (the prior resolution, now superseded): keep Supabase as the control plane, three tables only. It worked and was isolated (raw PostgREST fetch client, 10s timeout, no SDK dependency), but Amendment 2 removes an external system and the multi-installation rotation semantics it required; folding into Firestore places credential custody under the same crypto module, backup story, and erasure workflow as the rest of the data plane.

Resolved: ACCEPT (recommendation final), founder, 2026-07-06 (amendment brief, docs/ekoa-code-spec-amendment-brief.md).
Re-resolved: ALTERNATIVE - Supabase retired entirely; the three live tables fold into Firestore (the `credentials` singleton, the `orgs` collection, and the activation model), and the dead tables drop with the project. founder, 2026-07-06 (amendment 2, consolidated ledger, docs/ekoa-code-spec-amendment-2-consolidated-ledger.md).
...
- **Ciphertext compatibility.** Algorithm (AES-256-GCM) and ciphertext wire format are carried byte-compatible so migrated rows decrypt without a re-encryption pass, provided the key is preserved (reference/data-inventory.md §10.10). Encrypted-at-rest fields: the `credentials` singleton's model-provider credential (section 4.5), integration credentials, trigger HMAC secrets, captured browser `sessionState`, delegated Graph tokens in `app_sessions`.
...
Rejected alternative: GCP KMS envelope encryption from day one - stronger custody, but adds a cloud dependency to boot and to local dev while the threat model (single-process deployment under FIXED-8, encrypted volume) does not demand it yet.

Resolved: defaulted to recommendation per amendment Part 1, founder, 2026-07-06 (amendment brief, docs/ekoa-code-spec-amendment-brief.md).

## 4.8 Migration implications (summary; chapter 10 owns the procedure)

1. **App-data: filesystem import (default).** The production app-data backend as deployed by the committed ekoa-deploy pipeline is the **filesystem** backend (Q-02 factual half closed during the Amendment 2 session, RUN_LOG.md; section 4.9). The default migration is therefore a real import: production filesystem app-data is read and written through the collections engine by a one-shot import script - a deliverable of the implementation run (none exists in the old code; the old `app-data-migration.ts` files are filesystem-to-filesystem, verified). If a live-container re-confirmation at migration time finds the backend hand-set to `mongo`/Firestore instead, the import is skipped and the new stack's connection string simply repoints at the existing database (the cheaper branch). Either way, a live-container re-confirmation stays on the chapter 10 cutover checklist (criterion 6).
...
8. **Control plane migrates off Supabase** (P-08 re-resolved, section 4.5). The `standalone_credentials` row values import into the `credentials` Firestore singleton, re-encrypted under the carried `ENCRYPTION_KEY` via the one crypto module (section 4.7). `companies` and `installations` are **not** migrated - superseded by the org model and the activation model respectively. The Supabase project (all tables, including the nine dead ones) is retired entirely after the rollback window closes (chapter 10).
...
12. **Org creation** (Amendment 2, Part 4): migration creates one org per existing user, all on the default design system, with no brand carry; the old `company.json` singleton is archived, not imported (section 4.3.1). Every migrated user gains a required `orgId` pointing at their org, and the founder's account is seeded `super-admin`. In dependency order this runs before the `users` import (chapter 10 section 10.2). The founder re-runs Brasil Salomão's brand research and applies manual copies post-launch.

## 4.9 Conflicts and open questions recorded by this chapter

- **C1 / Q-02 - production backend value (factual half closed).** Reference code, provisioning scripts, and integration docs describe production app-data on Firestore (Mongo-compat, project `spatial-tempo-488909-s5`, db `ekoa-app-data`), but the literal production env value lives in the external ekoa-deploy repo. During the Amendment 2 session it was read there (verified against ekoa-deploy, recorded in RUN_LOG.md): the committed pipeline passes no `EKOA_APP_DATA_BACKEND` (and no Mongo URI) to the cortex container, so the switch defaults to `fs` and production-as-deployed runs the **filesystem** backend; the persistent volume mount `/opt/ekoa/cortex-data:/root/.ekoa` corroborates it. The earlier Firestore-in-prod reading reflected a hand-set `.env` on some host that the committed pipeline does not reproduce. Consequence: section 4.8 item 1 defaults to a **real filesystem import** through the engine, not a no-migration repoint. A hand-set override on the live container remains possible outside source, so **Q-02's remaining half is a live-container re-confirmation at migration time** (chapter 10 cutover criterion 6): if the live container proves to be on `mongo`/Firestore, the import is skipped and the connection string repoints (the cheaper branch). Resolved cutover-class: the factual value is closed here; the re-confirmation is a cutover-checklist ops action answered at cutover, not at run start (ch16 Q-02, mapped to chapter 10 criterion 6).
...
6. The API process refuses to boot when `ENCRYPTION_KEY` is absent (and enforces the JWT production guard per chapter 09); no default key constant exists in the repository (grep gate). There is no Supabase configuration to gate on - the control plane is on Firestore (section 4.5).
...
12. The `orgs` and `credentials` collections each appear in section 4.3.1 with a documented shape and tenancy, and their supporting index or singleton nature is stated in section 4.3.2; `users` carries a required `orgId` and an `active` boolean.
13. The `visibility` field (`private | org`) is present in the shared schemas for memories and artifacts (section 4.3.1); a memory or artifact persisted without a `visibility` value defaults to `private`.
14. The `teams` collection is absent from section 4.3.1 (DROPPED) and no importer targets it (chapter 10 section 10.8); a grep for a `teams` store in the new repo returns zero hits outside the chapter 11 glossary.
15. `EKOA_APP_DATA_BACKEND` and `getAppDataBackend` appear nowhere in the new repo (grep gate) - Firestore Mongo-compat is the only app-data backend (section 4.2.8).
...
Amended again 2026-07-06 per the consolidated-ledger amendment (Amendment 2, docs/ekoa-code-spec-amendment-2-consolidated-ledger.md): P-08 re-resolved - Supabase retired entirely, its control plane folded into Firestore (the `credentials` singleton for model-provider custody, the `orgs` collection for tenancy, and the activation model for installation/license facts), the section 4.5 heading and body rewritten with the `Re-resolved:` line and the verified license-server finding (none exists); the storage map, intro, and section 4.7 re-pointed off Supabase. The org model landed in the data layer: `users` gain required `orgId` and `active`; the `orgs` and `credentials` collections added; `company` superseded and archived; `teams` flipped to DROPPED; `visibility` (`private | org`) added to memories and artifacts; `integration_configs`, `activity_logs`, `knowledge_sources`, and `knowledge_uploads` re-scoped to the org; `automations`, `triggers`, and `automation_runs` gained a write-once denormalized `orgId` (stamped at creation) for the org-admin/Registo surface, with matching `{orgId}` indexes; the section 4.3.4 anonymisation/bridge surfaces and section 4.4.1 knowledge vault swept tenant->org and org-partitioned; `user_settings` noted as carrying the `build.verifyBuilds` and `memory.autoExtract` toggles; section 4.3.2 indexes updated (`users {orgId}`, `activity_logs {orgId, timestamp}`, `memories`/`artifacts` `{orgId}`, deny-list/bridge `{orgId}`). The dual app-data backend switch (`getAppDataBackend()`/`EKOA_APP_DATA_BACKEND`, filesystem driver) dropped - Firestore Mongo-compat is the only backend (section 4.2.8). Q-02's factual half closed (production-as-deployed is the filesystem backend, read from ekoa-deploy, RUN_LOG.md), so section 4.8 item 1 defaults to a real filesystem import; the migration section gained the control-plane and org-creation items (8, 12); section 4.10 acceptance criteria 6 and 12-15 updated.
