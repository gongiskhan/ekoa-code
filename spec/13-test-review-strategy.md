...
- **Cross-org adversarial suite (security addendum F3; Amendment 2).** An authenticated user of org A attempts every operation against org B's resources - specifically org B's brand, knowledge, memories, artifacts, automations, and Registo, across every domain, every route, and the collections engine - and the suite fails on anything but a clean 403/404 (never a 200, never a 500 that leaks existence). The collections engine gets the same treatment driven by a hostile manifest (one that names another org's collection, expresses an unscoped query, or claims an escalated capability). These are first-class per-PR members, not a one-off phase activity, and they escalate at whole-repo scope in the final security phase.
- **In-org sharing tests (Amendment 2, Part 4.12).** First-class members alongside the cross-org suite, exercising the `visibility: private | org` boundary within a single org: builder A reading builder B's private memory gets 404; builder A editing builder B's private artifact gets 403; an org-shared artifact is editable by any org member (git snapshots + restore + Registo cover the mutation); and a private item is invisible to the org admin too - the Registo metadata surface may show the item's existence, never its content.
- **Rate-limit and spend-cap tests (security addendum F4).** The per-org and per-user rate limits and spend caps at the LLM chokepoint (chapter 06; FIXED-14) are asserted deterministically against a stubbed clock and a fixture ledger: the request past the window limit is refused, the cap-reached state blocks a run at the pre-run allowance gate, and anomalous-burn alerting fires.
...
**Activation and feature suite classes (amendment 2026-07-06, Amendment 2).** Amendment 2 adds these named suite classes; where deterministic they are per-PR members (13.9).

- **Activation suite (Part 3).** Deactivation takes effect **immediately on all three admission planes** - the `/api/v1` auth middleware, the served-app plane (keyed on the artifact owner), and the bridge pairing plane - via the write-through activation map, with **no TTL wait**; the same deactivation revokes the user's outstanding tokens (P-03 set), asserted by a follow-up request failing. Envelope assertions: a deactivated user's request carries `ACCOUNT_DISABLED` (403), a billing-locked user's carries `BILLING_LOCKED` (402), each validated against the shared error envelope (chapter 03 section 3.3).
- **Memory auto-extract suite (P-12 re-resolved, Part 2.6).** Auto-extraction runs **asynchronously post-run** and never delays the terminal event (assert the terminal event fires without waiting on extraction); it **writes `visibility: 'private'`**; it emits a **Registo entry** for every automatic write; it books a ledger row attributed `user_work` with agentType `memory-extract`; and the **per-user toggle** is honored (default ON; with the toggle off, no extraction call and no write occur). The extractor is stubbed in the per-PR lane so the suite stays LLM-free; the real-model extraction rides the nightly lane.
- **Per-build verification suite (Part 6).** This is the *product feature's* test suite, explicitly **distinct from this chapter's own five-layer test strategy** - the feature verifies a user's app build; the strategy verifies ekoa-code. It asserts: the first-ever-build **ask-once dialog** stores the answer as the `build.verifyBuilds` setting (agent questions stay reserved for app ambiguity, never process); the **banner** is shown while testing (PT-PT copy verbatim); **incremental** behavior (a full acceptance pass on the artifact's first build, scoped tests of the change plus smoke on follow-ups); fix-forward is **bounded by the slice retry budget**; a failure to fix **completes the build with the honest visible note**; and the tokens book ledger rows attributed `user_work` with agentType `build-verify`.
- **Registo read-surface suite (Part 5).** The org-scoped activity read (`GET /api/v1/registo`, chapter 03 section 3.8.24) returns **metadata rows only** - who did what, when, usage per user - and **no chat/message body appears in any response**, across every filter (user, action type, date range); org-admin is scoped to its own org, super-admin reads across orgs (`?orgId=`).
...
| Billing and observability pages (teams deleted, Amendment 2) | Discovery then regression | Billing page spec pins usage display, credit purchase flow, overage toggle (rules already pinned by the ported billing gate unit test); observability gets behavior specs after a discovery pass. **Teams pages are deleted end to end (Amendment 2)** - no specs are owed. The activity read surface is **un-deferred by Amendment 2** (the org-scoped Registo read surface, ch03 §3.8.24): its coverage is the Registo read-surface suite (13.5, metadata-only) plus the Registo admin-page spec (ch12) |
...
- **Security-briefed review passes (security addendum F1-F2, amendment 2026-07-06).** On the same dual-model cadence but with a security charter distinct from the code-quality reviews above: (i) Claude Code's built-in security review over the full repo, findings triaged and looped back to implement like any other gate; (ii) a separately-briefed adversarial Codex pass charged specifically with authorization bypass, cross-org leakage, and injection paths - not the general adversarial review, a security one. Both run in the run's final security phase (chapter 14, Phase 12) over the whole repo; their verdicts are recorded in RUN_LOG.md and real findings block the gate. The cross-org adversarial suite (F3, 13.5) and the rate-limit/spend-cap tests (F4, 13.5) are the objective companions to these passes.
...
4. Contract suite (in-process app + `mongodb-memory-server`), ending with the schema-coverage gate and the protocol-parity gate (13.5); the deterministic security, privacy, and activation suite classes of 13.5 run in this lane - the cross-org adversarial suite (clean 403/404 only) and the in-org sharing tests, the activation suite (immediate write-through deactivation across the three admission planes plus token revocation, `ACCOUNT_DISABLED`/`BILLING_LOCKED` envelopes), the Registo read-surface metadata-only test, the memory auto-extract suite (extractor stubbed), the rate-limit and spend-cap tests, and the fake-daemon adversarial scenarios, tool_use round trip, streaming-straddle, prompt-cache byte-identical prefix, derived-output-only, and revoke-pairing suites, exercised against the fake daemon and a stubbed provider so they stay LLM-free.
...
11. The cross-org adversarial suite (13.5, security addendum F3; Amendment 2) exists as a first-class per-PR member: it enumerates an authenticated user of org A against org B's brand, knowledge, memories, artifacts, automations, and Registo for every domain and the collections engine under a hostile manifest, and it fails on any response other than 403/404 (verifiable by a deliberate red - a temporarily-unscoped repository query makes it red, logged in RUN_LOG.md). The in-org sharing tests are present too: builder A reads builder B's private memory (404), edits B's private artifact (403), an org-shared artifact is editable by an org member, and a private item is invisible to the org admin.
...
13. The fake-daemon adversarial suite (chapter 18 section 18.7) exists and is green: containment-violation, replay, expiry, cross-org-addressing, and forged-pairing requests are each rejected and ledgered as denials; the delegation round trip returns derived output only, the derived-output-only assertion confirms no raw local content in any hosted record, the correlation-id join succeeds, and the revoke-pairing kill switch refuses subsequent traffic.
14. The rate-limit and spend-cap tests at the chokepoint pass (13.5, security addendum F4: per-org/per-user limit refusal, cap-reached run block); the security-briefed review passes of 13.7 (Claude Code full-repo security review + adversarial security Codex pass) ran in the final phase with verdicts recorded in RUN_LOG.md.
15. The activation suite passes (13.5, Amendment 2): a deactivated user is refused on all three admission planes immediately (write-through, no TTL wait) and their outstanding tokens are revoked; `ACCOUNT_DISABLED` (403) and `BILLING_LOCKED` (402) each validate against the shared error envelope.
16. The memory auto-extract suite passes (13.5, Amendment 2): extraction is asynchronous post-run and does not delay the terminal event, writes `visibility: 'private'`, emits a Registo entry, books a `user_work`/`memory-extract` ledger row, and honors the per-user toggle (off = no extraction call and no write).
17. The per-build-verification suite passes (13.5, Amendment 2 - the product feature, distinct from this chapter's own strategy): the ask-once dialog stores `build.verifyBuilds`, the banner shows while testing, incremental behavior holds (first build full, follow-ups scoped + smoke), fix-forward is bounded by the slice retry budget, a fix failure completes with the honest note, and tokens book `user_work`/`build-verify` ledger rows.
18. The Registo read-surface suite passes (13.5, Amendment 2): `GET /api/v1/registo` returns metadata only - no chat/message body in any response - across all filters, org-admin scoped to its own org, super-admin across orgs.
...
Amended again 2026-07-06 per the consolidated-ledger amendment (Amendment 2, docs/ekoa-code-spec-amendment-2-consolidated-ledger.md): the cross-tenant adversarial suite was renamed the cross-org suite throughout (13.5, 13.7, 13.9, 13.11) and re-enumerated against org B's brand, knowledge, memories, artifacts, automations, and Registo, with the in-org sharing tests added as first-class members (private-memory 404, private-artifact-edit 403, org-shared artifact editable, private item invisible to the org admin). The activation suite was added (immediate write-through deactivation across the three admission planes plus token revocation, `ACCOUNT_DISABLED`/`BILLING_LOCKED` envelopes - 13.5). Suite classes were added for memory auto-extract (async post-run, writes private, Registo entry, `user_work`/`memory-extract`, terminal event not delayed, per-user toggle honored), per-build verification (the product feature, explicitly distinct from this chapter's own strategy - ask-once dialog, banner, incremental first-build-full/follow-up-scoped behavior, fix-forward bounded by the slice retry budget, honest note on failure, `user_work`/`build-verify`), and the org-scoped Registo read surface (metadata-only, no chat bodies). The billing/teams/observability gap row was rewritten (teams pages deleted end to end, no specs owed; the activity read surface un-deferred as the Registo read surface - 13.6); the per-PR and acceptance criteria were extended for the new suites (13.9, 13.11); and every live "tenant" was swept to "org".
