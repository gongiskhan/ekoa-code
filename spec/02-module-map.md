...
| `data/` | All persistence: Firestore client, domain stores (incl. `orgs`/activation and the encrypted `credentials` store), collections engine, crypto. |
...
Responsibility: all persistence. The Firestore client (Mongo-compatibility wire protocol via the `mongodb` driver), the platform domain stores (per P-05, ch04) including the `orgs` and activation stores, the collections engine that executes every user-app data operation from per-app manifests (FIXED-5), the model-credential store (the `credentials` collection, one singleton per environment, encrypted via the one crypto module; P-08 re-resolved, ch04 §4.5), and the single crypto module for encryption at rest (AES-256-GCM, key mandatory - unified in this rebuild from the two old implementations; ch09 owns the invariant). The activity store lives here with exactly one exported write function, `logActivity` - the single audit write path (FIXED-8; ch09 invariant 3). It is homed in `data/` deliberately: `data/` sits at the bottom of the import graph, so every writing module (`auth/`, `memory/`, `agents/`, `apps/`, `integrations/`, `knowledge/`, `billing/`, `services/`) can reach it within its existing import list; direct writes to the activity collection outside `logActivity` are grep-banned (ch09 9.3).
Ports: the app-data family including the Firestore backend and the storage contract that is already a clean seam (reference/carryover-audit.md A5); the crypto functions with the dev-fallback key removed (A4, `tools/crypto.ts` row and its security flag); the store schemas that follow the one storage decision (B14, P-05). The old Supabase client is not carried - the credential and installation custody it held moves to Firestore collections per ch04 §4.5 (P-08 re-resolved).
May import: `config.ts`.

### `auth/`

Responsibility: JWT sign/verify, the Express auth middleware, login, token refresh (P-03, ch03), device login, and first-boot admin seeding (invoked from `server.ts`). Produces the per-request user context consumed by every route. The middleware also consults the cached activation state - an in-memory map with write-through invalidation (the toggle write updates it synchronously; TTL is a safety net only; chapter 09) - so a deactivated user is refused at the first admission plane. Roles are three: `super-admin` (platform), `org-admin` (org administration), and `builder` (default member), with `orgId` carried in the JWT claims (chapters 04, 09).
...
- **Concern one - Anthropic chokepoint and attribution (FIXED-3; ch06 owns the mechanics).** Every Anthropic byte in the service flows through this module: `llm/client.ts` wraps (a) the Claude Agent SDK invocation used for user work, (b) the direct Messages REST call used for cheap classification, and (c) the metered pass-through for the ekoa-local gateway (the bridge provider endpoint, chapter 18 section 18.4); `llm/attribution.ts` tags every call at the call site as `user_work | platform | classifier`, recorded and metered from day one; `llm/router.ts` is the tier classifier. Central credential custody lives here because it exists solely to authenticate Anthropic calls: one credential per environment - the Firestore `credentials` singleton read through `data/` - with two auth modes as per-environment configuration, subscription OAuth and Anthropic API key (FIXED-8 as amended). The whole of the machinery is proactive refresh before expiry in oauth mode, refresh-and-retry-once on 401, and an alert on persistent failure; the multi-subscription rollover model (pools, rotation, health-scoring, selection) is not carried.
- **Concern two - the anonymisation pipeline (FIXED-8 as amended, FIXED-13; chapter 17 owns the full contract).** The amended FIXED-8 reads "anonymisation layer built in this run as part of the egress module (chapter 17)": every model-bound payload is tokenized on egress and de-tokenized on return through this one module, so anonymisation is a property of the single chokepoint, never a bypassable side path. The mechanism (detection, per-session in-memory vault, de-tokenization including tool_use argument blocks, hash-chained metadata-only audit) is core; the PT-PT ruleset and per-org deny-lists load as org configuration, mechanism core and ruleset composition (the Garrison line, FIXED-7). The pipeline is greenfield - the old `memory/anonymizer.ts` is dead code and is NOT its ancestor (chapter 11).
...
Responsibility: organizational memory - term-overlap resolver, prompt-section formatter, signals, migration and seeding, tiers and guardrail rules. Consumed by `agents/` (prompt injection) and `automation/` (the action cache is stored as memories). Scope in v1 per P-12 as re-resolved by Amendment 2 (ch05 section 5.8): CRUD plus resolver injection stay; automatic extraction ships ON by default (per-user toggle `memory.autoExtract`), but its FAST/Haiku-class post-run extraction call is made by `agents/` through `llm/` (agentType `memory-extract`, chapter 14 Phase 7B) - `memory/` only persists the result and consolidation stays deterministic code, so this module itself makes no model calls.
...
Responsibility: the user-app pipeline (ch07): esbuild bundling, manifest validation, scaffolding, the app registry with dist metadata and file watching, static serving with byte-compatible served-app context injection (`window.__ekoa`, `window.__EKOA_APP_ID` - the 37-spec legal e2e suite hangs on it, reference/test-audit.md §2.4), deterministic slug generation and the slug index, artifact bundle/fork/files/versions, artifact PDF, featured-artifact prebuild and update, app health scanning, and the artifact-backend worker runtime (Layer 2) whose model capability is billed to the artifact owner through `llm/` and whose notify capability is an injected callback (section 2.8). The build pipeline gains a per-build verification stage: default-on playwright-cli verification of built apps (chapter 07), integrated into the run lifecycle (chapter 05), its tokens attributed as `build-verify` `user_work` (chapter 06).
...
Responsibility: the composition root. Builds the Express app, mounts middleware (auth, billing gate, request logging into the single audit write path - FIXED-8), mounts every router, wires the injected seams of section 2.8, runs boot: config validation, activation-cache and revocation-set boot load (chapter 09 section 9.7.1; there is no boot-time license gate anymore), data-dir preparation, storage backend (Firestore Mongo-compat) connection with fail-fast on a bad connection string, first-boot admin seeding, index backfills, scheduled sweeps. The only file allowed to import everything.
...
Amended again 2026-07-06 per the consolidated-ledger amendment (Amendment 2, docs/ekoa-code-spec-amendment-2-consolidated-ledger.md): `data/` drops the Supabase client for the `orgs`/activation stores and the encrypted `credentials` singleton (P-08 re-resolved, ch04 §4.5); `llm/` concern one replaces the Supabase-backed managed-OAuth custody with central credential custody - the Firestore `credentials` singleton and two per-environment auth modes (subscription OAuth / API key), the multi-subscription rollover machinery dropped (FIXED-8 as amended); `routes/` drops `/teams`, renames `/company` to `/org` (adding `/orgs`), and gains `/registo`; `auth/` consults the write-through-invalidated activation cache and carries the three roles (super-admin / org-admin / builder) with `orgId` in the JWT claims (ch09); `apps/` gains the default-on per-build verification stage (`build-verify` user_work; chapters 05/06/07). The teams domain leaves the module map.
