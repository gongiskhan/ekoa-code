...
All eight classes execute in-process. There is no distributed queue and no worker pool - single multi-tenant process is FIXED-8, and today's system runs every job immediately in-process (reference/invisible-behaviors.md section 7.2: "No queue, no scheduler"). What changes is crash accountability, via the persistent job registry of RESOLVED (P-10), below.
...
- `CLAUDE_CODE_OAUTH_TOKEN` injected from the managed OAuth custody (Supabase-backed, ch04 4.5); the token fetch must succeed - there is no fallback (FIXED-8: managed OAuth only, no raw API keys, no `~/.claude` auth).
- `ANTHROPIC_API_KEY` and `ANTH_API_KEY` are **deleted** (raw API keys blocked - FIXED-8). `ANTHROPIC_BASE_URL` is never left to a user or environment value: per FIXED-13 it is set to point the subprocess at the internal `llm/` chokepoint, so the SDK's egress funnels through the anonymisation pipeline and metering before it reaches Anthropic (chapter 17.2; ch06). This is a repoint to the platform-controlled chokepoint, not a bypass - a user- or environment-supplied provider base URL never survives into the subprocess, and the chokepoint is the one sanctioned egress path.
...
### 5.4.2 Warm subprocess and auth retry (carried from section 2.7)

- A pre-warmed SDK subprocess handle (~20x faster first response) is maintained, single-use, and **invalidated whenever the OAuth token rotates** via the custody module's refresh callback.
- On a detected auth error the run forces one token refresh, invalidates the warm handle, and retries **once**. Transient provider markers retry with backoff `[5 s, 15 s]`. Exhaustion surfaces a real error - never a fake completion. User abort resolves silently.
- The custody module's rotation invariants (single-flight mutex, persist-first, never-delete-on-401, peer adoption, non-unref'd watchdog) are chapter 04/06 territory; this chapter's dependency is only: token available at run start, warm handle invalidated on rotation.

### 5.4.3 The OAuth model-access asymmetry (environmental constraint, recorded)

Managed OAuth direct REST authorizes the cheap tier only - the standard and expert tiers return 400 on that path; the SDK subprocess authorizes all tiers (reference/llm-usage-map.md section 2, Conflicts 7; confirmed live 2026-05-31). Consequences the rebuild inherits deliberately: the ekoa-local gateway clamps to the cheap tier on the wire; vision and the automation planner route their expert-tier one-shot calls through the SDK; and any future replacement of the Agent SDK must first re-solve strong-model access. Recorded here because it silently shapes every routing decision in this chapter.
...
| `MEMORY_AUTO_EXTRACT_ENABLED` | false | P-12 (5.8) |
...
5. `complete` event with result (a final-build error is appended as a user-visible note), artifact id, slug, and app URL; job -> `completed`.
6. Artifact -> `active` with a **merge** onto its existing data bag - a wholesale replace historically dropped customization and lineage fields (carried, section 7.2).
7. Fire-and-forget screenshot; assistant message persisted. (The fire-and-forget memory extraction that rode this step is dropped from the baseline - 5.9.1.)
...
## 5.8 RESOLVED (P-12) - memory system scope in v1

Memory CRUD (ch03 3.8.19) and resolver injection (5.5.2) ship on; **automatic extraction ships off by default** behind a config flag (`MEMORY_AUTO_EXTRACT_ENABLED`, 5.4.7), and consolidation is deterministic code (no model call). Rationale: auto-extraction is a background model call riding every chat turn, build, and assistant reply - roughly one cheap-tier call per conversation turn - that is not user-requested work; the product fully functions with it off, and its fate in the call-site map is "dropped from the platform baseline" (reference/llm-usage-map.md section 7, call 23; reference/invisible-behaviors.md section 11.2). If organizational memory learning is wanted later, it returns as an explicit user-invoked action, which reclassifies it as billable user work. The privacy-scrub patterns and the extraction pipeline shape are preserved in the codebase behind the flag, not deleted.

Rejected alternative: carry auto-extract on by default (status quo). Not taken - the run pipelines must know at build time whether the post-turn hook exists, and the baseline keeps it off.

Resolved: ACCEPT (recommendation final), founder, 2026-07-06 (amendment brief, docs/ekoa-code-spec-amendment-brief.md).

Independent of the extraction flag, the three deterministic memory writers - no model call in any of them (reference/invisible-behaviors.md section 11.6) - are **carried**, in scope under RESOLVED (P-12):
...
| Behavior | Reason |
|---|---|
| Post-turn memory auto-extraction (chat, build, assistant) | P-12: off by default; platform-overhead model call, fate "dropped from baseline" (reference/llm-usage-map.md section 7 call 23) |
| Assistant-reply memory co-op flag | rides the same extraction pipeline; already default-false today (reference/invisible-behaviors.md section 11.2) |
...
3. SDK environment test: the spawned subprocess env contains `CLAUDE_CODE_OAUTH_TOKEN`, does not contain `ANTHROPIC_API_KEY`, `ANTH_API_KEY`, or `CLAUDECODE`, and has `ANTHROPIC_BASE_URL` set to the internal chokepoint address (FIXED-13, 5.4.1) rather than any user or provider value; `settingSources` is empty; build runs get `HOME = projectDir`.
...
10. With the P-12 flag off (default), a full chat turn and a full build complete with zero extraction model calls (assertable from the attribution ledger, ch06).
...
Cross-references: diagram `spec/diagrams/04-agent-job` (FIXED-12); ch02 (module homes, injected seams); ch03 (endpoints, event unions, P-18); ch04 (jobs/automation_runs/memories collections, OAuth custody); ch06 (chokepoint, attribution, metering, tier table); ch07 (build pipeline mechanics invoked from 5.6.2); ch08 (content loader behind 5.5.1); ch09 (egress sanitization, secret guard); ch17 (anonymisation pipeline behind the chokepoint egress per FIXED-13; per-conversation vault, 5.4.8); ch18 (the `delegate_to_local` contract and fake-daemon harness, 5.4.8); ch13 (tests named above); ch16 (Q-04).
