 ...
 * assistant row whose traceId equals this run IS this run's persisted turn. Its ids get
...
export interface SendMessageOptions {
  ...
  /** C7 (BRIEF §5): 'voice' when the send originates from the mic - the api attaches the
...
export function ChatRuntimeProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleBuildSendMessage = useCallback(
    (message: string, opts?: { skipUserMessage?: boolean }) => {
      ...
      emitBuildKick();
      // Stamp the session job as initiated (status 'queued') alongside the panel
      // flip so panelContentFor renders the build variant (see handleBuildFirstMessage).
    ...
    [activeSessionId, pendingAttachments, execute, setSidePanelState, setSidePanelTab, setSessionJob, clearAttachments, sessionJobs, language, emitBuildKick]
  );
  ...
  const handleChatSend = useCallback(async (textArg: string, sendOpts?: SendMessageOptions) => {
    ...
    // and ONLY from the VISIBLE chip state at send time. Setting the chip is the job of the
    // typing/draft-time heuristic and the explicit affordances (a follow-up pill targets its
    // sheet; the X dismisses); no chip shown (auto with nothing set, or dismissed) means the
    // send is a fresh sheet - a revision is never silently inferred here (locked 6). Consumed
    // = reset to auto for the next turn.
    ...
    try {
      ...
      const { runId } = await api.chat.createRun({
        ...
        // voice-context note (output shaping only, never a shorter reply).
        ...(sendOpts?.source ? { source: sendOpts.source } : {}),
      });
      ...
      const handleComplete = (event: { result?: unknown; delegate?: { request: Record<string, unknown> } }) => {
        ...
        // fresh sheet's createdFromMessageId - never the chip's reviseSheetId. The lexical
        // runId from this run's closure - the shared ref is already cleared by finishStream()
        // and can belong to a newer run.
