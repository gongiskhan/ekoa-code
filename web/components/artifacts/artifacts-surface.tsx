...
function ArtifactCard({
  ...
  onOpenMenu: (pos: ActionMenuPosition) => void;
  ...
  // The item action menu's three triggers (surface contract 3.1): right-click
  // and long-press here; the always-visible "..." button in the footer below.
  // The hook's own contextmenu suppressor is dropped - the explicit handler
  // covers both the desktop right-click and the mobile synthesized event.
...
export function ArtifactsSurface({ host }: SurfaceProps) {
  const { pages_artifacts: a, common } = useTranslation();
  ...
  const [sortMenuPos, setSortMenuPos] = useState<ActionMenuPosition | null>(null);

  // Item action menu (surface contract 3): one definition, three triggers.
  const [cardMenu, setCardMenu] = useState<{
    artifact: ArtifactInstance;
    pos: ActionMenuPosition;
  } | null>(null);
  // Rename-from-menu dialog (the detail view keeps its inline TitleEditor).
  const [renamingArtifact, setRenamingArtifact] = useState<ArtifactInstance | null>(null);
  ...
  // ---- Item action menu context (surface contract 3.3) ----
  // One ActionDef list; the ui seam binds it to this surface's own affordances
  // (classic overlay/detail state). The OS shell binds the same list to
  // windows/desktop icons instead.
  const artifactActions = useMemo(() => buildArtifactActions(a), [a]);
  const menuCtx: ArtifactActionCtx | null = cardMenu
    ? {
        artifact: cardMenu.artifact,
        host,
        ui: {
          open: (artifact) => handleRun(artifact as ArtifactInstance),
          continueInChat: (artifact) => handleContinueWorking(artifact as ArtifactInstance),
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-canvas">
          ...
          <PageShell key="list" width="wide" testId="artifacts-page">
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {/* Sort - through the shared ActionMenu (no full-viewport
                      click-catcher backdrop; contract 3.2). */}
                  <button
                    onClick={(e) => {
            ...
            {featuredArtifacts.length > 0 && (
              ...
              {isLoading && instances.length === 0 ? (
                <div className="grid grid-cols-1 gap-4 @bp-md:grid-cols-2 @bp-lg:grid-cols-3 @bp-xl:grid-cols-4">
                ...
                <motion.div
                  ...
                  initial="hidden"
                  animate="show"
                  className="grid grid-cols-1 gap-4 @bp-md:grid-cols-2 @bp-lg:grid-cols-3 @bp-xl:grid-cols-4"
                >
                  {filtered.map((artifact) => (
                    <ArtifactCard
                      ...
                      onOpenMenu={(pos) => setCardMenu({ artifact, pos })}
                      copiedId={copiedId}
                      startingId={startingId}
                      stoppingId={stoppingId}
                    />
      ...
      {/* Item action menu - one definition for "...", right-click, long-press. */}
      <ActionMenu
        items={artifactActions}
        ctx={menuCtx as ArtifactActionCtx}
        position={cardMenu?.pos ?? null}
